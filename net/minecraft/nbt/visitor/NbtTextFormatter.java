...
public class NbtTextFormatter implements NbtElementVisitor {
   ...
   public void visitIntArray(NbtIntArray element) {
      ...
      for(int i = 0; i < is.length; ++i) {
         mutableText.append(" ").append((Text)(new LiteralText(String.valueOf(is[i]))).formatted(NUMBER_COLOR));
         if (i != is.length - 1) {
            mutableText.append(ENTRY_SEPARATOR);
         }
      }
   ...
   public void visitLongArray(NbtLongArray element) {
      ...
      for(int i = 0; i < ls.length; ++i) {
         ...
         mutableText.append(" ").append((Text)text2).append((Text)text);
         if (i != ls.length - 1) {
            mutableText.append(ENTRY_SEPARATOR);
         }
      }
   ...
   public void visitCompound(NbtCompound compound) {
      if (compound.isEmpty()) {
         ...
         for(Iterator iterator = ((Collection)collection).iterator(); iterator.hasNext(); mutableText.append((Text)mutableText2)) {
            ...
            mutableText2 = (new LiteralText(Strings.repeat(this.prefix, this.indentationLevel + 1))).append(escapeName(string)).append(KEY_VALUE_SEPARATOR).append(" ").append((new NbtTextFormatter(this.prefix, this.indentationLevel + 1)).apply(compound.get(string)));
            if (iterator.hasNext()) {

...
public class AdvancementUpdateS2CPacket implements Packet<ClientPlayPacketListener> {
   ...
   public AdvancementUpdateS2CPacket(boolean clearCurrent, Collection<Advancement> toEarn, Set<Identifier> toRemove, Map<Identifier, AdvancementProgress> toSetProgress) {
      ...
      this.toRemove = ImmutableSet.copyOf((Collection)toRemove);
   ...
   public void write(PacketByteBuf buf) {
      ...
      buf.writeMap(this.toEarn, PacketByteBuf::writeIdentifier, (bufx, task) -> {
         task.toPacket(bufx);
      });
      buf.writeCollection(this.toRemove, PacketByteBuf::writeIdentifier);
      buf.writeMap(this.toSetProgress, PacketByteBuf::writeIdentifier, (bufx, progress) -> {
         progress.toPacket(bufx);
      });
   }

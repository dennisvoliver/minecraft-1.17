   ...
   private class Matcher {
         ...
         for(int j = 0; j < i; ++j) {
            if (RecipeMatcher.this.inputs.get(this.requiredItems[j]) >= multiplier) {
               ...
               while(!this.ingredientItemLookup.isEmpty()) {
                  ...
                  int o;
                  for(o = 0; o < m; ++o) {
                     if (!this.isRequirementUnfulfilled(bl, o) && this.needsRequirement(bl, l, o) && this.checkRequirement(bl, l, o)) {
      ...
      private int getMaximumCrafts() {
         ...
         while(var2.hasNext()) {
            ...
            int k;
            for(IntListIterator var5 = ingredient.getMatchingItemIds().iterator(); var5.hasNext(); j = Math.max(j, RecipeMatcher.this.inputs.get(k))) {
               k = (Integer)var5.next();
            }

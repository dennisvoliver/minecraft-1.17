   ...
   BoundedIntUnaryOperator(@Nullable LootNumberProvider lootNumberProvider, @Nullable LootNumberProvider lootNumberProvider2) {
      ...
      if (lootNumberProvider == null) {
         if (lootNumberProvider2 == null) {
            ...
            this.applier = (context, value) -> {
               return Math.min(lootNumberProvider2.nextInt(context), value);
            };
            this.tester = (context, value) -> {
               return value <= lootNumberProvider2.nextInt(context);
            };
         }
      } else if (lootNumberProvider2 == null) {
         this.applier = (context, value) -> {
            return Math.max(lootNumberProvider.nextInt(context), value);
         };
         this.tester = (context, value) -> {
            return value >= lootNumberProvider.nextInt(context);
         };
      } else {
         this.applier = (context, value) -> {
            return MathHelper.clamp(value, lootNumberProvider.nextInt(context), lootNumberProvider2.nextInt(context));
         };
         this.tester = (context, value) -> {
            return value >= lootNumberProvider.nextInt(context) && value <= lootNumberProvider2.nextInt(context);
         };
      }
   ...
   public static class Serializer implements JsonDeserializer<BoundedIntUnaryOperator>, JsonSerializer<BoundedIntUnaryOperator> {
      public BoundedIntUnaryOperator deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) {
         if (jsonElement.isJsonPrimitive()) {
            ...
            LootNumberProvider lootNumberProvider = jsonObject.has("min") ? (LootNumberProvider)JsonHelper.deserialize(jsonObject, "min", jsonDeserializationContext, LootNumberProvider.class) : null;
            LootNumberProvider lootNumberProvider2 = jsonObject.has("max") ? (LootNumberProvider)JsonHelper.deserialize(jsonObject, "max", jsonDeserializationContext, LootNumberProvider.class) : null;

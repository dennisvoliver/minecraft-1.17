...
public class ResourcePackManager implements AutoCloseable {
   ...
   public ResourcePackManager(ResourceType type, ResourcePackProvider... providers) {
      this((string, text, bl, supplier, packResourceMetadata, insertionPosition, resourcePackSource) -> {
         return new ResourcePackProfile(string, text, bl, supplier, packResourceMetadata, type, insertionPosition, resourcePackSource);
      }, providers);
   }
   ...
   private Map<String, ResourcePackProfile> providePackProfiles() {
      ...
      while(var2.hasNext()) {
         ...
         resourcePackProvider.register((resourcePackProfile) -> {
            map.put(resourcePackProfile.getName(), resourcePackProfile);
         }, this.profileFactory);
      }

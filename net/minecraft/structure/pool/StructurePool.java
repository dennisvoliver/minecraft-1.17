...
public class StructurePool {
   ...
   public static final Codec<StructurePool> CODEC = RecordCodecBuilder.create((instance) -> {
      return instance.group(Identifier.CODEC.fieldOf("name").forGetter(StructurePool::getId), Identifier.CODEC.fieldOf("fallback").forGetter(StructurePool::getTerminatorsId), Codec.mapPair(StructurePoolElement.CODEC.fieldOf("element"), Codec.intRange(1, 150).fieldOf("weight")).codec().listOf().fieldOf("elements").forGetter((structurePool) -> {
   ...
   public StructurePool(Identifier id, Identifier terminatorsId, List<Pair<StructurePoolElement, Integer>> elementCounts) {
      ...
      Iterator var4 = elementCounts.iterator();
   ...
   public StructurePool(Identifier id, Identifier terminatorsId, List<Pair<Function<StructurePool.Projection, ? extends StructurePoolElement>, Integer>> elementCounts, StructurePool.Projection projection) {
      this.id = id;
   ...
   public int getHighestY(StructureManager structureManager) {
      if (this.highestY == Integer.MIN_VALUE) {
         this.highestY = this.elements.stream().filter((structurePoolElement) -> {
   ...
   public List<StructurePoolElement> getElementIndicesInRandomOrder(Random random) {
      return ImmutableList.copyOf((Object[])((StructurePoolElement[])ObjectArrays.shuffle((StructurePoolElement[])this.elements.toArray(new StructurePoolElement[0]), random)));
   }
   ...
   static {
      REGISTRY_CODEC = RegistryElementCodec.of(Registry.STRUCTURE_POOL_KEY, CODEC);
   }
   ...
   public static enum Projection implements StringIdentifiable {
      ...
      public static final Codec<StructurePool.Projection> CODEC = StringIdentifiable.createCodec(StructurePool.Projection::values, StructurePool.Projection::getById);

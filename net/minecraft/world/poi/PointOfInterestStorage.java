...
public class PointOfInterestStorage extends SerializingRegionBasedStorage<PointOfInterestSet> {
   ...
   public Stream<BlockPos> getSortedPositions(Predicate<PointOfInterestType> typePredicate, Predicate<BlockPos> posPredicate, BlockPos pos, int radius, PointOfInterestStorage.OccupationStatus occupationStatus) {
      return this.getPositions(typePredicate, posPredicate, pos, radius, occupationStatus).sorted(Comparator.comparingDouble((blockPos2) -> {
   ...
   public Optional<BlockPos> method_34712(Predicate<PointOfInterestType> predicate, Predicate<BlockPos> predicate2, BlockPos blockPos, int i, PointOfInterestStorage.OccupationStatus occupationStatus) {
      return this.getInCircle(predicate, blockPos, i, occupationStatus).map(PointOfInterest::getPos).filter(predicate2).min(Comparator.comparingDouble((blockPos2) -> {
   ...
   public boolean releaseTicket(BlockPos pos) {
      return (Boolean)this.get(ChunkSectionPos.toLong(pos)).map((pointOfInterestSet) -> {
         return pointOfInterestSet.releaseTicket(pos);
      }).orElseThrow(() -> {
   ...
   public Optional<PointOfInterestType> getType(BlockPos pos) {
      return this.get(ChunkSectionPos.toLong(pos)).flatMap((pointOfInterestSet) -> {
   ...
   public int method_35155(BlockPos blockPos) {
      return (Integer)this.get(ChunkSectionPos.toLong(blockPos)).map((pointOfInterestSet) -> {
         return pointOfInterestSet.method_35157(blockPos);
      }).orElse(0);
   }
   ...
   public void preloadChunks(WorldView world, BlockPos pos, int radius) {
      ChunkSectionPos.stream(new ChunkPos(pos), Math.floorDiv(radius, 16), this.world.getBottomSectionCoord(), this.world.getTopSectionCoord()).map((chunkSectionPos) -> {

...
public class PointOfInterestType {
   ...
   public static final Predicate<PointOfInterestType> IS_USED_BY_PROFESSION = (poiType) -> {
      return ((Set)VILLAGER_WORKSTATIONS.get()).contains(poiType);
   };
   public static final Predicate<PointOfInterestType> ALWAYS_TRUE = (poiType) -> {
      return true;
   };
   private static final Set<BlockState> BED_STATES;
   private static final Set<BlockState> CAULDRON_STATES;
   ...
   private PointOfInterestType(String id, Set<BlockState> blockStates, int ticketCount, int searchDistance) {
      ...
      this.blockStates = ImmutableSet.copyOf((Collection)blockStates);
      this.ticketCount = ticketCount;
      this.completionCondition = (poiType) -> {
         return poiType == this;
      };
      this.searchDistance = searchDistance;
   }
   ...
   static {
      ...
      CAULDRON_STATES = (Set)ImmutableList.of(Blocks.CAULDRON, Blocks.LAVA_CAULDRON, Blocks.WATER_CAULDRON, Blocks.POWDER_SNOW_CAULDRON).stream().flatMap((block) -> {
         return block.getStateManager().getStates().stream();
      }).collect(ImmutableSet.toImmutableSet());
      ...
      LEATHERWORKER = register("leatherworker", CAULDRON_STATES, 1, 1);
      ...
      LIGHTNING_ROD = register("lightning_rod", getAllStatesOf(Blocks.LIGHTNING_ROD), 0, 1);

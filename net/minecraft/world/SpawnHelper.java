...
public final class SpawnHelper {
   ...
   public static void spawn(ServerWorld world, WorldChunk chunk, SpawnHelper.Info info, boolean spawnAnimals, boolean spawnMonsters, boolean rareSpawn) {
      ...
      for(int var8 = 0; var8 < var7; ++var8) {
         ...
         if ((spawnAnimals || !spawnGroup.isPeaceful()) && (spawnMonsters || spawnGroup.isPeaceful()) && (rareSpawn || !spawnGroup.isRare()) && info.isBelowCap(spawnGroup)) {
            Objects.requireNonNull(info);
            SpawnHelper.Checker var10003 = info::test;
            Objects.requireNonNull(info);
                  ...
                  if (isAcceptableSpawnPosition(world, chunk, mutable, f)) {
                     ...
                     if (canSpawn(world, group, structureAccessor, chunkGenerator, spawnEntry, mutable, f) && checker.test(spawnEntry.type, mutable, chunk)) {
                        ...
                        if (isValidSpawn(world, mobEntity, f)) {
                           entityData = mobEntity.initialize(world, world.getLocalDifficulty(mobEntity.getBlockPos()), SpawnReason.NATURAL, entityData, (NbtCompound)null);
                           ++j;
                           ++p;
            ...
            for(int p = 0; p < k; ++p) {
               ...
               for(int q = 0; !bl && q < 4; ++q) {
                  ...
                  if (spawnEntry.type.isSummonable() && canSpawn(SpawnRestriction.getLocation(spawnEntry.type), world, blockPos, spawnEntry.type)) {
                     ...
                     Entity entity2;
                     try {
                        entity2 = spawnEntry.type.create(world.toServerWorld());
                     ...
                     if (entity2 instanceof MobEntity) {
                        ...
                        if (mobEntity.canSpawn(world, SpawnReason.CHUNK_GENERATION) && mobEntity.canSpawn(world)) {
                           entityData = mobEntity.initialize(world, world.getLocalDifficulty(mobEntity.getBlockPos()), SpawnReason.CHUNK_GENERATION, entityData, (NbtCompound)null);
                           world.spawnEntityAndPassengers(mobEntity);
                           bl = true;
                        }
                     }

...
public interface CollisionView extends BlockView {
   ...
   default boolean isSpaceEmpty(Box box) {
      return this.isSpaceEmpty((Entity)null, box, (e) -> {
         return true;
      });
   }

   default boolean isSpaceEmpty(Entity entity) {
      return this.isSpaceEmpty(entity, entity.getBoundingBox(), (e) -> {
         return true;
      });
   }

   default boolean isSpaceEmpty(Entity entity, Box box) {
      return this.isSpaceEmpty(entity, box, (e) -> {
         return true;
      });
   }
   ...
   default Optional<Vec3d> findClosestCollision(@Nullable Entity entity, VoxelShape shape, Vec3d target, double x, double y, double z) {
      if (shape.isEmpty()) {
         ...
         VoxelShape voxelShape = (VoxelShape)this.getBlockCollisions(entity, box).flatMap((collision) -> {
            return collision.getBoundingBoxes().stream();
         }).map((boxx) -> {
            return boxx.expand(x / 2.0D, y / 2.0D, z / 2.0D);

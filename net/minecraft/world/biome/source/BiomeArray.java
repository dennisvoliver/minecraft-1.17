...
public class BiomeArray implements BiomeAccess.Storage {
   ...
   public BiomeArray(IndexedIterable<Biome> biomes, HeightLimitView world, int[] ids) {
      ...
      for(int j = 0; j < this.data.length; ++j) {
         int k = ids[j];
         Biome biome = (Biome)biomes.get(k);
         if (biome == null) {
            if (i == -1) {
               i = j;
            }

            this.data[j] = (Biome)biomes.get(0);
         } else {
            this.data[j] = biome;
         }
      }
      ...
      if (i != -1) {
         LOGGER.warn((String)"Invalid biome data received, starting from {}: {}", (Object)i, (Object)Arrays.toString(ids));
      }
   ...
   public BiomeArray(IndexedIterable<Biome> biomes, HeightLimitView world, ChunkPos chunkPos, BiomeSource biomeSource, @Nullable int[] is) {
      ...
      for(int l = 0; l < this.data.length; ++l) {
         if (is != null && l < is.length) {
            this.data[l] = (Biome)biomes.get(is[l]);
         }
   ...
   public Biome getBiomeForNoiseGen(int biomeX, int biomeY, int biomeZ) {
      ...
      int j = MathHelper.clamp((int)(biomeY - this.field_28126), (int)0, (int)this.field_28127);
   ...
   static {
      ...
      DEFAULT_LENGTH = 1 << HORIZONTAL_SECTION_COUNT + HORIZONTAL_SECTION_COUNT + DimensionType.SIZE_BITS_Y - 2;
   }
}

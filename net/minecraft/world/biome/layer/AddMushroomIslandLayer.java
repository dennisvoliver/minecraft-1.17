...
public enum AddMushroomIslandLayer implements DiagonalCrossSamplingLayer {
   ...
   public int sample(LayerRandomnessSource context, int sw, int se, int ne, int nw, int center) {
      return BiomeLayers.isShallowOcean(center) && BiomeLayers.isShallowOcean(nw) && BiomeLayers.isShallowOcean(sw) && BiomeLayers.isShallowOcean(ne) && BiomeLayers.isShallowOcean(se) && context.nextInt(100) == 0 ? BiomeIds.MUSHROOM_FIELDS : center;
   }
}

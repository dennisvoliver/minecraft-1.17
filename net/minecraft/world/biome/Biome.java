      ...
      for(int j = 0; j < i; ++j) {
         ...
         if (structureAccessor.shouldGenerateStructures()) {
            ...
            for(Iterator var15 = list2.iterator(); var15.hasNext(); ++k) {
               ...
               Supplier supplier = () -> {
                  Optional var10000 = registry2.getKey(structureFeature).map(Object::toString);
                  Objects.requireNonNull(structureFeature);
                  return (String)var10000.orElseGet(structureFeature::toString);
               };
         ...
         if (list.size() > j) {
            for(Iterator var27 = ((List)list.get(j)).iterator(); var27.hasNext(); ++k) {
               Supplier<ConfiguredFeature<?, ?>> supplier2 = (Supplier)var27.next();
               ConfiguredFeature<?, ?> configuredFeature = (ConfiguredFeature)supplier2.get();
               Supplier<String> supplier3 = () -> {
                  Optional var10000 = registry.getKey(configuredFeature).map(Object::toString);
                  Objects.requireNonNull(configuredFeature);
                  return (String)var10000.orElseGet(configuredFeature::toString);
               };
               random.setDecoratorSeed(populationSeed, k, j);
               ...
               try {
                  ...
                  var10000.add("Description", supplier3::get);

...
public class SectionedEntityCache<T extends EntityLike> {
   ...
   public void forEachInBox(Box box, Consumer<EntityTrackingSection<T>> action) {
      int i = ChunkSectionPos.getSectionCoord(box.minX - 2.0D);
      int j = ChunkSectionPos.getSectionCoord(box.minY - 2.0D);
      int k = ChunkSectionPos.getSectionCoord(box.minZ - 2.0D);
      int l = ChunkSectionPos.getSectionCoord(box.maxX + 2.0D);
      int m = ChunkSectionPos.getSectionCoord(box.maxY + 2.0D);
      int n = ChunkSectionPos.getSectionCoord(box.maxZ + 2.0D);
      ...
      for(int o = i; o <= l; ++o) {
         ...
         while(longIterator.hasNext()) {
            ...
            if (s >= j && s <= m && t >= k && t <= n) {
               EntityTrackingSection<T> entityTrackingSection = (EntityTrackingSection)this.trackingSections.get(r);
   ...
   private static <T extends EntityLike> Predicate<T> intersecting(Box box) {
      return (entityLike) -> {
         return entityLike.getBoundingBox().intersects(box);
      };
   }

   public void forEachIntersects(Box box, Consumer<T> action) {
      this.forEachInBox(box, (entityTrackingSection) -> {
         entityTrackingSection.forEach(intersecting(box), action);
      });
   }

   public <U extends T> void forEachIntersects(TypeFilter<T, U> filter, Box box, Consumer<U> action) {
      this.forEachInBox(box, (entityTrackingSection) -> {
         entityTrackingSection.forEach(filter, intersecting(box), action);
      });
   }

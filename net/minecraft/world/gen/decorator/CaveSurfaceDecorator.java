...
public class CaveSurfaceDecorator extends Decorator<CaveSurfaceDecoratorConfig> {
   ...
   public Stream<BlockPos> getPositions(DecoratorContext decoratorContext, Random random, CaveSurfaceDecoratorConfig caveSurfaceDecoratorConfig, BlockPos blockPos) {
      Optional<CaveSurface> optional = CaveSurface.create(decoratorContext.getWorld(), blockPos, caveSurfaceDecoratorConfig.searchRange, AbstractBlock.AbstractBlockState::isAir, (blockState) -> {
         return blockState.getMaterial().isSolid();
      });
      if (!optional.isPresent()) {
         return Stream.of();
      } else {
         OptionalInt optionalInt = caveSurfaceDecoratorConfig.surface == VerticalSurfaceType.CEILING ? ((CaveSurface)optional.get()).getCeilingHeight() : ((CaveSurface)optional.get()).getFloorHeight();

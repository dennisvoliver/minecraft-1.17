...
public class FlatChunkGeneratorConfig {
   ...
   private static DataResult<FlatChunkGeneratorConfig> checkHeight(FlatChunkGeneratorConfig config) {
      ...
      return i > DimensionType.MAX_HEIGHT ? DataResult.error("Sum of layer heights is > " + DimensionType.MAX_HEIGHT, (Object)config) : DataResult.success(config);
   }
   ...
   public Biome createBiome() {
      ...
      list3 = this.getLayerBlocks();

      for(i = 0; i < list3.size(); ++i) {
         BlockState blockState = (BlockState)list3.get(i);
         if (!Heightmap.Type.MOTION_BLOCKING.getBlockPredicate().test(blockState)) {
            list3.set(i, (Object)null);
   ...
   public static FlatChunkGeneratorConfig getDefaultConfig(Registry<Biome> biomeRegistry) {
      StructuresConfig structuresConfig = new StructuresConfig(Optional.of(StructuresConfig.DEFAULT_STRONGHOLD), Maps.newHashMap(ImmutableMap.of(StructureFeature.VILLAGE, (StructureConfig)StructuresConfig.DEFAULT_STRUCTURES.get(StructureFeature.VILLAGE))));

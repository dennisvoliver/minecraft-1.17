...
public class StructureAccessor {
   ...
   public StructureAccessor forRegion(ChunkRegion region) {
      if (region.toServerWorld() != this.world) {
         ...
         throw new IllegalStateException("Using invalid feature manager (source level: " + var10002 + ", region: " + region);
      } else {
   ...
   public Stream<? extends StructureStart<?>> getStructuresWithChildren(ChunkSectionPos pos, StructureFeature<?> feature) {
      return this.world.getChunk(pos.getSectionX(), pos.getSectionZ(), ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences(feature).stream().map((long_) -> {
         return ChunkSectionPos.from(new ChunkPos(long_), this.world.getBottomSectionCoord());
      }).map((posx) -> {

         ...
         for(v = k; v <= l; ++v) {
            for(w = 3; w >= -1; --w) {
               for(x = p; x <= q; ++x) {
                  ...
                  } else if (blockPos3.getY() >= structureWorldAccess.getBottomY() && !structureWorldAccess.getBlockState(blockPos3.down()).getMaterial().isSolid()) {
         ...
         for(v = 0; v < 2; ++v) {
            for(w = 0; w < 3; ++w) {
               ...
               if (structureWorldAccess.isAir(blockPos4)) {
                  ...
                  if (ad == 1) {
                     this.setBlockStateIf(structureWorldAccess, blockPos4, StructurePiece.orientateChest(structureWorldAccess, blockPos4, Blocks.CHEST.getDefaultState()), predicate);

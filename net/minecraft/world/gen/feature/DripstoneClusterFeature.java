...
public class DripstoneClusterFeature extends Feature<DripstoneClusterFeatureConfig> {
      ...
      if (optional.isPresent()) {
         ...
         if (optionalInt.isPresent() || optionalInt2.isPresent()) {
            ...
            if (optionalInt.isPresent() && bl2 && !this.isLava(world, pos.withY(optionalInt.getAsInt()))) {
               q = config.dripstoneBlockLayerThickness.get(random);
               this.placeDripstoneBlocks(world, pos.withY(optionalInt.getAsInt()), q, Direction.UP);
               int l;
               if (optionalInt3.isPresent()) {
                  l = Math.min(height, optionalInt.getAsInt() - optionalInt3.getAsInt());
               } else {
                  l = height;
               }

               n = this.getHeight(random, localX, localZ, density, l, config);
            } else {
               n = 0;
            }
            ...
            if (optionalInt3.isPresent() && bl3 && !this.isLava(world, pos.withY(optionalInt3.getAsInt()))) {
               z = config.dripstoneBlockLayerThickness.get(random);
               this.placeDripstoneBlocks(world, pos.withY(optionalInt3.getAsInt()), z, Direction.DOWN);
               q = Math.max(0, n + MathHelper.nextBetween(random, -config.maxStalagmiteStalactiteHeightDiff, config.maxStalagmiteStalactiteHeightDiff));
            } else {
               q = 0;
            }

            int aa;
            if (optionalInt.isPresent() && optionalInt3.isPresent() && optionalInt.getAsInt() - n <= optionalInt3.getAsInt() + q) {
               int r = optionalInt3.getAsInt();
               int s = optionalInt.getAsInt();
               int t = Math.max(s - n, r + 1);
               int u = Math.min(r + q, s - 1);
            ...
            boolean bl4 = random.nextBoolean() && z > 0 && aa > 0 && caveSurface2.getOptionalHeight().isPresent() && z + aa == caveSurface2.getOptionalHeight().getAsInt();
            if (optionalInt.isPresent()) {
               DripstoneHelper.generatePointedDripstone(world, pos.withY(optionalInt.getAsInt() - 1), Direction.DOWN, z, bl4);
            }

            if (optionalInt3.isPresent()) {
               DripstoneHelper.generatePointedDripstone(world, pos.withY(optionalInt3.getAsInt() + 1), Direction.UP, aa, bl4);
            }
   ...
   private int getHeight(Random random, int localX, int localZ, float density, int height, DripstoneClusterFeatureConfig config) {
      if (random.nextFloat() > density) {
         ...
         float f = (float)MathHelper.clampedLerpFromProgress((double)i, 0.0D, (double)config.maxDistanceFromCenterAffectingHeightBias, (double)height / 2.0D, 0.0D);
   ...
   private double dripstoneChance(int radiusX, int radiusZ, int localX, int localZ, DripstoneClusterFeatureConfig config) {
      ...
      int k = Math.min(i, j);
      return MathHelper.clampedLerpFromProgress((double)k, 0.0D, (double)config.maxDistanceFromCenterAffectingChanceOfDripstoneColumn, (double)config.chanceOfDripstoneColumnAtMaxDistanceFromCenter, 1.0D);
   }

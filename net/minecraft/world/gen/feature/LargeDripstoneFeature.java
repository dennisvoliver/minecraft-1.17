...
public class LargeDripstoneFeature extends Feature<LargeDripstoneFeatureConfig> {
      ...
      if (!DripstoneHelper.canGenerate(structureWorldAccess, blockPos)) {
         ...
         if (optional.isPresent() && optional.get() instanceof CaveSurface.Bounded) {
            ...
            if (bounded.getHeight() < 4) {
               ...
               LargeDripstoneFeature.DripstoneGenerator dripstoneGenerator = createGenerator(blockPos.withY(bounded.getCeiling() - 1), false, random, k, largeDripstoneFeatureConfig.stalactiteBluntness, largeDripstoneFeatureConfig.heightScale);
               LargeDripstoneFeature.DripstoneGenerator dripstoneGenerator2 = createGenerator(blockPos.withY(bounded.getFloor() + 1), true, random, k, largeDripstoneFeatureConfig.stalagmiteBluntness, largeDripstoneFeatureConfig.heightScale);
               LargeDripstoneFeature.WindModifier windModifier2;
               if (dripstoneGenerator.generateWind(largeDripstoneFeatureConfig) && dripstoneGenerator2.generateWind(largeDripstoneFeatureConfig)) {
                  windModifier2 = new LargeDripstoneFeature.WindModifier(blockPos.getY(), random, largeDripstoneFeatureConfig.windSpeed);
               } else {
                  windModifier2 = LargeDripstoneFeature.WindModifier.create();
               }
               ...
               if (bl) {
                  dripstoneGenerator.generate(structureWorldAccess, random, windModifier2);
               }

               if (bl2) {
                  dripstoneGenerator2.generate(structureWorldAccess, random, windModifier2);
               }
   ...
   private static LargeDripstoneFeature.DripstoneGenerator createGenerator(BlockPos pos, boolean isStalagmite, Random random, int scale, FloatProvider bluntness, FloatProvider heightScale) {
   ...
   private void method_35360(StructureWorldAccess structureWorldAccess, BlockPos blockPos, CaveSurface.Bounded bounded, LargeDripstoneFeature.WindModifier windModifier) {
   ...
   static final class WindModifier {
      ...
      WindModifier(int y, Random random, FloatProvider wind) {
         ...
         this.wind = new Vec3d((double)(MathHelper.cos(g) * f), 0.0D, (double)(MathHelper.sin(g) * f));
      }

...
public class RuinedPortalFeature extends StructureFeature<RuinedPortalFeatureConfig> {
   ...
   static int getFloorHeight(Random random, ChunkGenerator chunkGenerator, RuinedPortalStructurePiece.VerticalPlacement verticalPlacement, boolean airPocket, int height, int blockCountY, BlockBox box, HeightLimitView world) {
      ...
      if (verticalPlacement == RuinedPortalStructurePiece.VerticalPlacement.IN_NETHER) {
         if (airPocket) {
            q = MathHelper.nextBetween(random, 32, 100);
         } else if (random.nextFloat() < 0.5F) {
            q = MathHelper.nextBetween(random, 27, 29);
         } else {
            q = MathHelper.nextBetween(random, 29, 100);
         }
      } else {
         int n;
         if (verticalPlacement == RuinedPortalStructurePiece.VerticalPlacement.IN_MOUNTAIN) {
            n = height - blockCountY;
            q = choosePlacementHeight(random, 70, n);
         } else if (verticalPlacement == RuinedPortalStructurePiece.VerticalPlacement.UNDERGROUND) {
            n = height - blockCountY;
            q = choosePlacementHeight(random, 15, n);
         } else if (verticalPlacement == RuinedPortalStructurePiece.VerticalPlacement.PARTLY_BURIED) {
            q = height - blockCountY + MathHelper.nextBetween(random, 2, 8);
         } else {
            q = height;
         }
      }

      List<BlockPos> list = ImmutableList.of(new BlockPos(box.getMinX(), 0, box.getMinZ()), new BlockPos(box.getMaxX(), 0, box.getMinZ()), new BlockPos(box.getMinX(), 0, box.getMaxZ()), new BlockPos(box.getMaxX(), 0, box.getMaxZ()));
      List<VerticalBlockSample> list2 = (List)list.stream().map((blockPos) -> {
         return chunkGenerator.getColumnSample(blockPos.getX(), blockPos.getZ(), world);
      ...
      for(r = q; r > 15; --r) {
         int s = 0;
         mutable.set(0, r, 0);
         Iterator var15 = list2.iterator();

         while(var15.hasNext()) {
            VerticalBlockSample verticalBlockSample = (VerticalBlockSample)var15.next();

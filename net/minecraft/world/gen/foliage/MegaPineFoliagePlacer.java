...
public class MegaPineFoliagePlacer extends FoliagePlacer {
   public static final Codec<MegaPineFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> {
      return fillFoliagePlacerFields(instance).and((App)IntProvider.createValidatingCodec(0, 24).fieldOf("crown_height").forGetter((placer) -> {
   ...
   protected void generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, TreeFeatureConfig config, int trunkHeight, FoliagePlacer.TreeNode treeNode, int foliageHeight, int radius, int offset) {
      ...
      for(int j = blockPos.getY() - foliageHeight + offset; j <= blockPos.getY() + offset; ++j) {
         ...
         this.generateSquare(world, replacer, random, config, new BlockPos(blockPos.getX(), j, blockPos.getZ()), n, 0, treeNode.isGiantTrunk());
         i = l;
      }

...
public class LargeOakTrunkPlacer extends TrunkPlacer {
   public static final Codec<LargeOakTrunkPlacer> CODEC = RecordCodecBuilder.create((instance) -> {
      return fillTrunkPlacerFields(instance).apply(instance, (Function3)(LargeOakTrunkPlacer::new));
   });
   ...
   public List<FoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
      ...
      while(var37.hasNext()) {
         LargeOakTrunkPlacer.BranchPosition branchPosition = (LargeOakTrunkPlacer.BranchPosition)var37.next();
   ...
   private boolean makeOrCheckBranch(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, BlockPos startPos, BlockPos branchPos, boolean make, TreeFeatureConfig config) {
      if (!make && Objects.equals(startPos, branchPos)) {
         ...
         for(int j = 0; j <= i; ++j) {
            ...
            if (make) {
               TrunkPlacer.getAndSetState(world, replacer, random, blockPos2, config, (state) -> {
                  return (BlockState)state.with(PillarBlock.AXIS, this.getLogAxis(startPos, blockPos2));
               });
   ...
   private void makeBranches(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int treeHeight, BlockPos startPos, List<LargeOakTrunkPlacer.BranchPosition> branchPositions, TreeFeatureConfig config) {
      Iterator var8 = branchPositions.iterator();

      while(var8.hasNext()) {
         LargeOakTrunkPlacer.BranchPosition branchPosition = (LargeOakTrunkPlacer.BranchPosition)var8.next();

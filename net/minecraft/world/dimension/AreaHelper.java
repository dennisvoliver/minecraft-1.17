...
public class AreaHelper {
   ...
   public void createPortal() {
      ...
      BlockPos.iterate(this.lowerCorner, this.lowerCorner.offset(Direction.UP, this.height - 1).offset(this.negativeDir, this.width - 1)).forEach((blockPos) -> {
         this.world.setBlockState(blockPos, blockState, Block.NOTIFY_LISTENERS | Block.FORCE_STATE);
      });
   }
   ...
   public static Vec3d entityPosInPortal(PortalUtil.Rectangle portalRect, Direction.Axis portalAxis, Vec3d entityPos, EntityDimensions entityDimensions) {
      ...
      if (d > 0.0D) {
         ...
         h = MathHelper.clamp(MathHelper.getLerpProgress(entityPos.getComponentAlongAxis(portalAxis) - (double)f, 0.0D, d), 0.0D, 1.0D);
      } else {
         h = 0.5D;
      }
      ...
      if (e > 0.0D) {
         ...
         j = MathHelper.clamp(MathHelper.getLerpProgress(entityPos.getComponentAlongAxis(axis2) - (double)blockPos.getComponentAlongAxis(axis2), 0.0D, e), 0.0D, 1.0D);
      } else {
         j = 0.0D;
      }

      axis2 = portalAxis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
      double k = entityPos.getComponentAlongAxis(axis2) - ((double)blockPos.getComponentAlongAxis(axis2) + 0.5D);

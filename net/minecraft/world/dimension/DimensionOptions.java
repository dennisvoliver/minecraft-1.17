...
public final class DimensionOptions {
   public static final Codec<DimensionOptions> CODEC = RecordCodecBuilder.create((instance) -> {
      return instance.group(DimensionType.REGISTRY_CODEC.fieldOf("type").flatXmap(Codecs.method_37352(), Codecs.method_37352()).forGetter(DimensionOptions::getDimensionTypeSupplier), ChunkGenerator.CODEC.fieldOf("generator").forGetter(DimensionOptions::getChunkGenerator)).apply(instance, (App)instance.stable(DimensionOptions::new));
   });
   ...
   public static boolean hasDefaultSettings(long seed, SimpleRegistry<DimensionOptions> options) {
      List<Entry<RegistryKey<DimensionOptions>, DimensionOptions>> list = Lists.newArrayList((Iterable)options.getEntries());
   ...
   static {
      OVERWORLD = RegistryKey.of(Registry.DIMENSION_KEY, new Identifier("overworld"));
      NETHER = RegistryKey.of(Registry.DIMENSION_KEY, new Identifier("the_nether"));
      END = RegistryKey.of(Registry.DIMENSION_KEY, new Identifier("the_end"));

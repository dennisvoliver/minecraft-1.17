...
public class UpgradeData {
   ...
   public UpgradeData(NbtCompound nbt, HeightLimitView world) {
      ...
      for(int var6 = 0; var6 < var10; ++var6) {
         EightWayDirection eightWayDirection = var9[var6];
   ...
   private static void upgradeSide(WorldChunk chunk, EightWayDirection side) {
      ...
      if (chunk.getUpgradeData().sidesToUpgrade.remove(side)) {
         ...
         Iterator var18 = BlockPos.iterate(k, world.getBottomY(), m, l, world.getTopY() - 1, n).iterator();
   ...
   private void upgradeCenter(WorldChunk chunk) {
      ...
      for(n = 0; n < this.centerIndicesToUpgrade.length; ++n) {
         ...
         int[] is = this.centerIndicesToUpgrade[n];
      ...
      for(n = 0; n < this.centerIndicesToUpgrade.length; ++n) {
         if (this.centerIndicesToUpgrade[n] != null) {
            LOGGER.warn((String)"Discarding update data for section {} for chunk ({} {})", (Object)worldAccess.sectionIndexToCoord(n), chunkPos.x, chunkPos.z);
         }
   ...
   public NbtCompound toNbt() {
      ...
      for(j = 0; j < this.centerIndicesToUpgrade.length; ++j) {
         ...
         if (this.centerIndicesToUpgrade[j] != null && this.centerIndicesToUpgrade[j].length != 0) {
            nbtCompound2.putIntArray(string, this.centerIndicesToUpgrade[j]);
         }
      }
   ...
   private static enum BuiltinLogic implements UpgradeData.Logic {

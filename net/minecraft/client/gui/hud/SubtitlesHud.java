...
public class SubtitlesHud extends DrawableHelper implements SoundInstanceListener {
   ...
   public void render(MatrixStack matrices) {
      ...
      if (this.enabled && !this.entries.isEmpty()) {
         ...
         Vec3d vec3d2 = (new Vec3d(0.0D, 0.0D, -1.0D)).rotateX(-this.client.player.getPitch() * 0.017453292F).rotateY(-this.client.player.getYaw() * 0.017453292F);
         Vec3d vec3d3 = (new Vec3d(0.0D, 1.0D, 0.0D)).rotateX(-this.client.player.getPitch() * 0.017453292F).rotateY(-this.client.player.getYaw() * 0.017453292F);
         ...
         for(iterator = this.entries.iterator(); iterator.hasNext(); ++i) {
            ...
            int q = p << 16 | p << 8 | p;
            matrices.push();
            matrices.translate((double)((float)this.client.getWindow().getScaledWidth() - (float)l * 1.0F - 2.0F), (double)((float)(this.client.getWindow().getScaledHeight() - 30) - (float)(i * (m + 1)) * 1.0F), 0.0D);

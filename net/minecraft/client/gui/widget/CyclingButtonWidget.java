...
public class CyclingButtonWidget<T> extends PressableWidget implements OrderableTooltip {
   ...
   public void appendNarrations(NarrationMessageBuilder builder) {
      ...
      if (this.active) {
         ...
         if (this.isFocused()) {
            builder.put(NarrationPart.USAGE, (Text)(new TranslatableText("narration.cycle_button.usage.focused", new Object[]{text})));
         } else {
            builder.put(NarrationPart.USAGE, (Text)(new TranslatableText("narration.cycle_button.usage.hovered", new Object[]{text})));
         }
      }
   ...
   public static CyclingButtonWidget.Builder<Boolean> onOffBuilder(Text on, Text off) {
      return (new CyclingButtonWidget.Builder((value) -> {
         return value ? on : off;
   ...
   private interface Values<T> {
      ...
      static <T> CyclingButtonWidget.Values<T> of(List<T> values) {
         final List<T> list = ImmutableList.copyOf((Collection)values);
      ...
      static <T> CyclingButtonWidget.Values<T> of(final BooleanSupplier alternativeToggle, List<T> defaults, List<T> alternatives) {
         final List<T> list = ImmutableList.copyOf((Collection)defaults);
         final List<T> list2 = ImmutableList.copyOf((Collection)alternatives);
         return new CyclingButtonWidget.Values<T>() {
            public List<T> getCurrent() {
   ...
   public static class Builder<T> {
      ...
      private T value;
      private final Function<T, Text> valueToText;
      private CyclingButtonWidget.TooltipFactory<T> tooltipFactory = (value) -> {
      ...
      public final CyclingButtonWidget.Builder<T> values(T... values) {
         return this.values((List)ImmutableList.copyOf(values));
      }

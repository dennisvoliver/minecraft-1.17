...
public class BundleTooltipComponent implements TooltipComponent {
   ...
   private void drawSlot(int x, int y, int index, boolean shouldBlock, TextRenderer textRenderer, MatrixStack matrices, ItemRenderer itemRenderer, int z, TextureManager textureManager) {
   ...
   private void drawOutline(int x, int y, int columns, int rows, MatrixStack matrices, int z, TextureManager textureManager) {
      ...
      int j;
      for(j = 0; j < columns; ++j) {
         this.draw(matrices, x + 1 + j * 18, y, z, textureManager, BundleTooltipComponent.Sprite.BORDER_HORIZONTAL_TOP);
         this.draw(matrices, x + 1 + j * 18, y + rows * 20, z, textureManager, BundleTooltipComponent.Sprite.BORDER_HORIZONTAL_BOTTOM);
      }

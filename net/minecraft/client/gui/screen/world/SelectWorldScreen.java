...
public class SelectWorldScreen extends Screen {
   ...
   protected void init() {
      ...
      this.selectButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 - 154, this.height - 52, 150, 20, new TranslatableText("selectWorld.select"), (button) -> {
         this.levelList.getSelectedAsOptional().ifPresent(WorldListWidget.Entry::play);
      }));
      this.addDrawableChild(new ButtonWidget(this.width / 2 + 4, this.height - 52, 150, 20, new TranslatableText("selectWorld.create"), (button) -> {
         this.client.openScreen(CreateWorldScreen.create(this));
      }));
      this.editButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 - 154, this.height - 28, 72, 20, new TranslatableText("selectWorld.edit"), (button) -> {
         this.levelList.getSelectedAsOptional().ifPresent(WorldListWidget.Entry::edit);
      }));
      this.deleteButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 - 76, this.height - 28, 72, 20, new TranslatableText("selectWorld.delete"), (button) -> {
         this.levelList.getSelectedAsOptional().ifPresent(WorldListWidget.Entry::deleteIfConfirmed);
      }));
      this.recreateButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 + 4, this.height - 28, 72, 20, new TranslatableText("selectWorld.recreate"), (button) -> {

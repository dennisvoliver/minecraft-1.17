...
public class MoreOptionsDialog implements Drawable {
   ...
   public void init(CreateWorldScreen parent, MinecraftClient client, TextRenderer textRenderer) {
      ...
      this.mapFeaturesButton = (CyclingButtonWidget)parent.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.generatorOptions.shouldGenerateStructures()).narration((button) -> {
         return ScreenTexts.joinSentences(button.getGenericNarrationMessage(), new TranslatableText("selectWorld.mapFeatures.info"));
      }).build(i, 100, 150, 20, new TranslatableText("selectWorld.mapFeatures"), (button, generateStructures) -> {
         this.generatorOptions = this.generatorOptions.toggleGenerateStructures();
      }));
      this.mapFeaturesButton.visible = false;
      this.mapTypeButton = (CyclingButtonWidget)parent.addDrawableChild(CyclingButtonWidget.builder(GeneratorType::getTranslationKey).values((List)GeneratorType.VALUES.stream().filter(GeneratorType::isNotDebug).collect(Collectors.toList()), GeneratorType.VALUES).narration((button) -> {
         return button.getValue() == GeneratorType.AMPLIFIED ? ScreenTexts.joinSentences(button.getGenericNarrationMessage(), AMPLIFIED_INFO_TEXT) : button.getGenericNarrationMessage();
      ...
      Optional var10000 = this.generatorType;
      CyclingButtonWidget var10001 = this.mapTypeButton;
      Objects.requireNonNull(var10001);
      var10000.ifPresent(var10001::setValue);
      this.mapTypeButton.visible = false;
      this.field_28001 = (ButtonWidget)parent.addDrawableChild(new ButtonWidget(j, 100, 150, 20, ScreenTexts.composeGenericOptionText(new TranslatableText("selectWorld.mapType"), CUSTOM_TEXT), (button) -> {
      }));
      this.field_28001.active = false;
      this.field_28001.visible = false;
      this.customizeTypeButton = (ButtonWidget)parent.addDrawableChild(new ButtonWidget(j, 120, 150, 20, new TranslatableText("selectWorld.customizeType"), (button) -> {
      ...
      this.bonusItemsButton = (CyclingButtonWidget)parent.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.generatorOptions.hasBonusChest() && !parent.hardcore).build(i, 151, 150, 20, new TranslatableText("selectWorld.bonusItems"), (button, bonusChest) -> {
         this.generatorOptions = this.generatorOptions.toggleBonusChest();
      }));
      this.bonusItemsButton.visible = false;
      this.importSettingsButton = (ButtonWidget)parent.addDrawableChild(new ButtonWidget(i, 185, 150, 20, new TranslatableText("selectWorld.import_worldgen_settings"), (button) -> {
         String string = TinyFileDialogs.tinyfd_openFileDialog((CharSequence)SELECT_SETTINGS_FILE_TEXT.getString(), (CharSequence)null, (PointerBuffer)null, (CharSequence)null, false);
         if (string != null) {
            DynamicRegistryManager.Impl impl = DynamicRegistryManager.create();
            ResourcePackManager resourcePackManager = new ResourcePackManager(ResourceType.SERVER_DATA, new ResourcePackProvider[]{new VanillaDataPackProvider(), new FileResourcePackProvider(parent.getDataPackTempDir().toFile(), ResourcePackSource.PACK_SOURCE_WORLD)});
            ...
            try {
               ...
               LOGGER.error((String)"Error loading data packs when importing world settings", (Throwable)var15);
            ...
            RegistryOps<JsonElement> registryOps = RegistryOps.method_36574(JsonOps.INSTANCE, serverResourceManager2.getResourceManager(), impl);
            ...
            try {
               ...
               dataResult3 = DataResult.error("Failed to parse file: " + var17.getMessage());
            }
   ...
   void loadDatapacks(ServerResourceManager serverResourceManager) {
      ...
      dataResult.resultOrPartial(Util.addPrefix("Error parsing worldgen settings after loading data packs: ", var10002::error)).ifPresent((generatorOptions) -> {
         this.generatorOptions = generatorOptions;

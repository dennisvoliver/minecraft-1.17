...
public class RecipeAlternativesWidget extends DrawableHelper implements Drawable, Element {
   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      if (this.visible) {
         ...
         matrices.push();
         matrices.translate(0.0D, 0.0D, 170.0D);
   ...
   class FurnaceAlternativeButtonWidget extends RecipeAlternativesWidget.AlternativeButtonWidget {
      ...
      protected void alignRecipe(Recipe<?> recipe) {
         ItemStack[] itemStacks = ((Ingredient)recipe.getIngredients().get(0)).getMatchingStacksClient();
         this.slots.add(new RecipeAlternativesWidget.AlternativeButtonWidget.InputSlot(10, 10, itemStacks));
      }
   }

   @Environment(EnvType.CLIENT)
   private class AlternativeButtonWidget extends ClickableWidget implements RecipeGridAligner<Ingredient> {
      final Recipe<?> recipe;
      ...
      public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
         ...
         matrixStack.push();
         matrixStack.translate((double)(this.x + 2), (double)(this.y + 2), 125.0D);
         Iterator var8 = this.slots.iterator();

         while(var8.hasNext()) {
            RecipeAlternativesWidget.AlternativeButtonWidget.InputSlot inputSlot = (RecipeAlternativesWidget.AlternativeButtonWidget.InputSlot)var8.next();
            matrixStack.push();
            matrixStack.translate((double)inputSlot.y, (double)inputSlot.x, 0.0D);

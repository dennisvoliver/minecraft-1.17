...
public class ScreenNarrator {
   ...
   final Map<ScreenNarrator.PartIndex, ScreenNarrator.Message> narrations = Maps.newTreeMap(Comparator.comparing((partIndex) -> {
      return partIndex.part;
   }).thenComparing((partIndex) -> {
      return partIndex.messageIndex;
   }));
   ...
   @Environment(EnvType.CLIENT)
   private class MessageBuilder implements NarrationMessageBuilder {
      ...
      public void put(NarrationPart part, Narration<?> narration) {
         ((ScreenNarrator.Message)ScreenNarrator.this.narrations.computeIfAbsent(new ScreenNarrator.PartIndex(part, this.messageIndex), (partIndex) -> {
            return new ScreenNarrator.Message();

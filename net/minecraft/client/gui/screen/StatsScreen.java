   ...
   private class ItemStatsListWidget extends AlwaysSelectedEntryListWidget<StatsScreen.ItemStatsListWidget.Entry> {
      ...
      protected void renderDecorations(MatrixStack matrices, int mouseX, int mouseY) {
         if (mouseY >= this.top && mouseY <= this.bottom) {
            StatsScreen.ItemStatsListWidget.Entry entry = (StatsScreen.ItemStatsListWidget.Entry)this.method_37019();
      ...
      private class Entry extends AlwaysSelectedEntryListWidget.Entry<StatsScreen.ItemStatsListWidget.Entry> {
         ...
         public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            for(i = 0; i < StatsScreen.this.itemStats.blockStatTypes.size(); ++i) {
               ...
               if (this.field_33830 instanceof BlockItem) {
                  stat2 = ((StatType)StatsScreen.this.itemStats.blockStatTypes.get(i)).getOrCreateStat(((BlockItem)this.field_33830).getBlock());
               } else {
                  stat2 = null;
               }
            ...
            for(i = 0; i < StatsScreen.this.itemStats.itemStatTypes.size(); ++i) {
               this.render(matrices, ((StatType)StatsScreen.this.itemStats.itemStatTypes.get(i)).getOrCreateStat(this.field_33830), x + StatsScreen.this.getColumnX(i + StatsScreen.this.itemStats.blockStatTypes.size()), y, index % 2 == 0);
            }
         ...
         public Text method_37006() {
            return new TranslatableText("narrator.select", new Object[]{this.field_33830.getName()});
         }
      }
   }

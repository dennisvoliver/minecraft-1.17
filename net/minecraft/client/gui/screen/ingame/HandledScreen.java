   ...
   public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
      ...
      for(int k = 0; k < this.handler.slots.size(); ++k) {
         ...
         if (this.isPointOverSlot(slot, (double)mouseX, (double)mouseY) && slot.isEnabled()) {
            ...
            p = slot.x;
            q = slot.y;
            method_33285(matrices, p, q, this.getZOffset());
         }
      }
      ...
      if (!itemStack.isEmpty()) {
         int n = true;
         p = this.touchDragStack.isEmpty() ? 8 : 16;
         String string = null;
         ...
         this.drawItem(itemStack, mouseX - i - 8, mouseY - j - p, string);
      }
      ...
      if (!this.touchDropReturningStack.isEmpty()) {
         ...
         p = this.touchDropOriginSlot.x - this.touchDropX;
         q = this.touchDropOriginSlot.y - this.touchDropY;
         int r = this.touchDropX + (int)((float)p * f);
         int s = this.touchDropY + (int)((float)q * f);
         this.drawItem(this.touchDropReturningStack, r, s, (String)null);
      }
      ...
      if (this.doubleClicking && slot != null && button == 0 && this.handler.canInsertIntoSlot(ItemStack.EMPTY, slot)) {
         ...
         if (this.touchDragSlotStart != null && this.client.options.touchscreen) {
            if (button == 0 || button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
               ...
               if (k != GLFW.GLFW_KEY_UNKNOWN && !this.touchDragStack.isEmpty() && bl3) {
            ...
            if (this.client.options.keyPickItem.matchesMouse(button)) {
               ...
               bl3 = k != -999 && (InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT) || InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), GLFW.GLFW_KEY_RIGHT_SHIFT));
               if (bl3) {
                  this.quickMovingStack = slot != null && slot.hasStack() ? slot.getStack().copy() : ItemStack.EMPTY;
               }

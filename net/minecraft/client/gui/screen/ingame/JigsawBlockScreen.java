...
public class JigsawBlockScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addDrawableChild(new SliderWidget(this.width / 2 - 154, 180, 100, 20, LiteralText.EMPTY, 0.0D) {
         {
            this.updateMessage();
         }
      ...
      this.field_33958 = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 + 54, 180, 100, 20, new TranslatableText("jigsaw_block.generate"), (button) -> {
         this.onDone();
         this.generate();
      }));
      this.doneButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 - 4 - 150, 210, 150, 20, ScreenTexts.DONE, (button) -> {
         this.onDone();
      }));

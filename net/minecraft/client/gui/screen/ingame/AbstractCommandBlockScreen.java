...
public abstract class AbstractCommandBlockScreen extends Screen {
   ...
   protected void init() {
      ...
      this.doneButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 - 4 - 150, this.height / 4 + 120 + 12, 150, 20, ScreenTexts.DONE, (button) -> {
         this.commitAndClose();
      }));
      this.cancelButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 + 4, this.height / 4 + 120 + 12, 150, 20, ScreenTexts.CANCEL, (button) -> {
         this.onClose();
      }));
      boolean bl = this.getCommandExecutor().isTrackingOutput();
      this.toggleTrackingOutputButton = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.onOffBuilder(new LiteralText("O"), new LiteralText("X")).initially(bl).omitKeyText().build(this.width / 2 + 150 - 20, this.getTrackOutputButtonHeight(), 20, 20, new TranslatableText("advMode.trackOutput"), (button, trackOutput) -> {

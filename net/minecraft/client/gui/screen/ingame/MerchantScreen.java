...
public class MerchantScreen extends HandledScreen<MerchantScreenHandler> {
   ...
   protected void init() {
      ...
      for(int l = 0; l < 7; ++l) {
         this.offers[l] = (MerchantScreen.WidgetButtonPage)this.addDrawableChild(new MerchantScreen.WidgetButtonPage(i + 5, k, l, (button) -> {
   ...
   protected void drawForeground(MatrixStack matrices, int mouseX, int mouseY) {
      ...
      if (i > 0 && i <= 5 && ((MerchantScreenHandler)this.handler).isLeveled()) {
         Text text = this.title.shallowCopy().append(SEPARATOR_TEXT).append((Text)(new TranslatableText("merchant.level." + i)));
      ...
      public void renderToolTip(MatrixStack matrices, int mouseX, int mouseY) {
         if (this.hovered && ((MerchantScreenHandler)MerchantScreen.this.handler).getRecipes().size() > this.index + MerchantScreen.this.indexStartOffset) {
            ...
            if (mouseX < this.x + 20) {
               ...
               itemStack3 = ((TradeOffer)((MerchantScreenHandler)MerchantScreen.this.handler).getRecipes().get(this.index + MerchantScreen.this.indexStartOffset)).getSellItem();
               MerchantScreen.this.renderTooltip(matrices, itemStack3, mouseX, mouseY);
            }
         }

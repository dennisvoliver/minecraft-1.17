...
public class CyclingOption<T> extends Option {
   ...
   private Function<MinecraftClient, CyclingButtonWidget.TooltipFactory<T>> tooltips = (client) -> {
      return (value) -> {
   ...
   public static <T> CyclingOption<T> create(String key, List<T> values, Function<T, Text> valueToText, Function<GameOptions, T> getter, CyclingOption.Setter<T> setter) {
      return new CyclingOption(key, getter, setter, () -> {
   ...
   public static <T> CyclingOption<T> create(String key, Supplier<List<T>> valuesSupplier, Function<T, Text> valueToText, Function<GameOptions, T> getter, CyclingOption.Setter<T> setter) {
      return new CyclingOption(key, getter, setter, () -> {
         return CyclingButtonWidget.builder(valueToText).values((List)valuesSupplier.get());
      });
   }
   ...
   public static <T> CyclingOption<T> create(String key, List<T> defaults, List<T> alternatives, BooleanSupplier alternativeToggle, Function<T, Text> valueToText, Function<GameOptions, T> getter, CyclingOption.Setter<T> setter) {
      return new CyclingOption(key, getter, setter, () -> {
         return CyclingButtonWidget.builder(valueToText).values(alternativeToggle, defaults, alternatives);
      });
   }
   ...
   public static CyclingOption<Boolean> create(String key, Text on, Text off, Function<GameOptions, Boolean> getter, CyclingOption.Setter<Boolean> setter) {
      return new CyclingOption(key, getter, setter, () -> {
         return CyclingButtonWidget.onOffBuilder(on, off);
      });
   }
   ...
   public ClickableWidget createButton(GameOptions options, int x, int y, int width) {
      CyclingButtonWidget.TooltipFactory<T> tooltipFactory = (CyclingButtonWidget.TooltipFactory)this.tooltips.apply(MinecraftClient.getInstance());
      return ((CyclingButtonWidget.Builder)this.buttonBuilderFactory.get()).tooltip(tooltipFactory).initially(this.getter.apply(options)).build(x, y, width, 20, this.getDisplayPrefix(), (button, value) -> {

...
public class RealmsGetServerDetailsTask extends LongRunningTask {
   ...
   public void run() {
      ...
      RealmsServerAddress realmsServerAddress2;
      try {
         realmsServerAddress2 = this.join();
         ...
         case 6006:
            ...
            setScreen((Screen)(bl ? new RealmsBrokenWorldScreen(this.lastScreen, this.mainScreen, this.server.id, this.server.worldType == RealmsServer.WorldType.MINIGAME) : new RealmsGenericErrorScreen(new TranslatableText("mco.brokenworld.nonowner.title"), new TranslatableText("mco.brokenworld.nonowner.error"), this.lastScreen)));
            return;
         default:
            this.error(var5.toString());
            LOGGER.error((String)"Couldn't connect to world", (Throwable)var5);
            return;
         }
         ...
         LOGGER.error((String)"Couldn't connect to world", (Throwable)var7);
         this.error(var7.getLocalizedMessage());
         return;
      }

      boolean bl2 = realmsServerAddress2.resourcePackUrl != null && realmsServerAddress2.resourcePackHash != null;
      Screen screen = bl2 ? this.createResourcePackConfirmationScreen(realmsServerAddress2, this::createConnectingScreen) : this.createConnectingScreen(realmsServerAddress2);

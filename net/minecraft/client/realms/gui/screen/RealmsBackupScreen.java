...
public class RealmsBackupScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.downloadButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width - 135, row(1), 120, 20, new TranslatableText("mco.backup.button.download"), (button) -> {
         this.downloadClicked();
      }));
      this.restoreButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width - 135, row(3), 120, 20, new TranslatableText("mco.backup.button.restore"), (button) -> {
         this.restoreClicked(this.selectedBackup);
      }));
      this.changesButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width - 135, row(5), 120, 20, new TranslatableText("mco.backup.changes.tooltip"), (button) -> {
         this.client.openScreen(new RealmsBackupInfoScreen(this, (Backup)this.backups.get(this.selectedBackup)));
         this.selectedBackup = -1;
      }));
      this.addDrawableChild(new ButtonWidget(this.width - 100, this.height - 35, 85, 20, ScreenTexts.BACK, (button) -> {
   ...
   private void downloadWorldData() {
      ...
      var10000.openScreen(new RealmsLongRunningMcoTaskScreen(var10003, new DownloadTask(this.serverData.id, this.slotId, var10008 + " (" + ((RealmsWorldOptions)this.serverData.slots.get(this.serverData.activeSlot)).getSlotName(this.serverData.activeSlot) + ")", this)));
   }
   ...
   @Environment(EnvType.CLIENT)
   private class BackupObjectSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry<RealmsBackupScreen.BackupObjectSelectionListEntry> {
      ...
      public Text method_37006() {
         return new TranslatableText("narrator.select", new Object[]{this.mBackup.lastModifiedDate.toString()});
      }
   }
}

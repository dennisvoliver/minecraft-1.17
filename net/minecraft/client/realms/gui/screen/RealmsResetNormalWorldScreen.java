...
public class RealmsResetNormalWorldScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addDrawableChild(CyclingButtonWidget.builder(RealmsWorldGeneratorType::getText).values((Object[])RealmsWorldGeneratorType.values()).initially(this.generatorType).build(this.width / 2 - 102, row(4), 205, 20, new TranslatableText("selectWorld.mapType"), (button, generatorType) -> {
         this.generatorType = generatorType;
      }));
      this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.mapFeatures).build(this.width / 2 - 102, row(6) - 2, 205, 20, new TranslatableText("selectWorld.mapFeatures"), (button, mapFeatures) -> {
         this.mapFeatures = mapFeatures;
      }));
      this.addDrawableChild(new ButtonWidget(this.width / 2 - 102, row(12), 97, 20, this.parentTitle, (button) -> {
         this.callback.accept(new ResetWorldInfo(this.seedEdit.getText(), this.generatorType, this.mapFeatures));
      }));
      this.addDrawableChild(new ButtonWidget(this.width / 2 + 8, row(12), 97, 20, ScreenTexts.BACK, (button) -> {
         this.onClose();
      }));
   }

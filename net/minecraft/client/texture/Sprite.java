...
public class Sprite implements AutoCloseable {
   ...
   protected Sprite(SpriteAtlasTexture atlas, Sprite.Info info, int maxLevel, int atlasWidth, int atlasHeight, int x, int y, NativeImage nativeImage) {
      ...
      try {
         try {
            ...
            crashReport = CrashReport.create(var12, "Generating mipmaps for frame");
         ...
         crashReport = CrashReport.create(var13, "Applying mipmap");
         crashReportSection = crashReport.addElement("Sprite being mipmapped");
         Identifier var10002 = this.id;
         Objects.requireNonNull(var10002);
         crashReportSection.add("Sprite name", var10002::toString);
   ...
   private Sprite.Animation createAnimation(Sprite.Info info, int nativeImageWidth, int nativeImageHeight, int maxLevel) {
      ...
      animationResourceMetadata.forEachFrame((index, time) -> {
         list.add(new Sprite.AnimationFrame(index, time));
      });
      int m;
      if (list.isEmpty()) {
         for(m = 0; m < k; ++m) {
            list.add(new Sprite.AnimationFrame(m, animationResourceMetadata.getDefaultFrameTime()));
         }
      } else {
         m = 0;
         IntSet intSet = new IntOpenHashSet();

         for(Iterator iterator = list.iterator(); iterator.hasNext(); ++m) {
            Sprite.AnimationFrame animationFrame = (Sprite.AnimationFrame)iterator.next();
            boolean bl = true;
            if (animationFrame.time <= 0) {
               LOGGER.warn((String)"Invalid frame duration on sprite {} frame {}: {}", (Object)this.id, m, animationFrame.time);
               bl = false;
            }

            if (animationFrame.index < 0 || animationFrame.index >= k) {
               LOGGER.warn((String)"Invalid frame index on sprite {} frame {}: {}", (Object)this.id, m, animationFrame.index);
               bl = false;
            }
         ...
         if (is.length > 0) {
            LOGGER.warn((String)"Unused frames in sprite {}: {}", (Object)this.id, (Object)Arrays.toString(is));
         }
      }
      ...
      if (list.size() <= 1) {
         ...
         return new Sprite.Animation(ImmutableList.copyOf((Collection)list), i, interpolation);
      }
   }
   ...
   public String toString() {
      ...
      return "TextureAtlasSprite{name='" + var10000 + "', frameCount=" + this.getFrameCount() + ", x=" + this.x + ", y=" + this.y + ", height=" + this.height + ", width=" + this.width + ", u0=" + this.uMin + ", u1=" + this.uMax + ", v0=" + this.vMin + ", v1=" + this.vMax + "}";
   }
   ...
   public boolean isPixelTransparent(int frame, int x, int y) {
      ...
      return (this.images[0].getPixelColor(i, j) >> 24 & 255) == 0;
   }
   ...
   @Environment(EnvType.CLIENT)
   private class Animation implements TextureTickListener, AutoCloseable {
      int frameIndex;
      int frameTicks;
   ...
   final class Interpolation implements AutoCloseable {
            ...
            for(int k = 0; k < this.images.length; ++k) {
               ...
               for(int n = 0; n < m; ++n) {
                  for(int o = 0; o < l; ++o) {
                     ...
                     this.images[k].setPixelColor(o, n, p & -16777216 | r << 16 | s << 8 | t);
                  }
               }
            }
      ...
      private int getPixelColor(Sprite.Animation animation, int frameIndex, int layer, int x, int y) {
         return Sprite.this.images[layer].getPixelColor(x + (animation.getFrameX(frameIndex) * Sprite.this.width >> layer), y + (animation.getFrameY(frameIndex) * Sprite.this.height >> layer));
      }

...
public final class NativeImage implements AutoCloseable {
   ...
   public static NativeImage read(@Nullable NativeImage.Format format, ByteBuffer byteBuffer) throws IOException {
      if (format != null && !format.isWriteable()) {
         ...
         try {
            ...
            var7 = new NativeImage(format == null ? NativeImage.Format.getFormat(intBuffer3.get(0)) : format, intBuffer.get(0), intBuffer2.get(0), true, MemoryUtil.memAddress(byteBuffer2));
   ...
   public void writeFile(Path path) throws IOException {
      if (!this.format.isWriteable()) {
         ...
         this.checkAllocated();
         SeekableByteChannel writableByteChannel = Files.newByteChannel(path, WRITE_TO_FILE_OPEN_OPTIONS);

...
public interface BlockListChecker {
   ...
   static BlockListChecker create() {
      final ImmutableList<Predicate<String>> immutableList = (ImmutableList)Streams.stream((Iterable)ServiceLoader.load(BlockListSupplier.class)).map(BlockListSupplier::createBlockList).filter(Objects::nonNull).collect(ImmutableList.toImmutableList());

...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
   ...
   private void pushOutOfBlocks(double x, double z) {
      ...
      if (this.wouldCollideAt(blockPos)) {
         ...
         for(int var16 = 0; var16 < var15; ++var16) {
            ...
            double g = direction2.getAxis().choose(d, 0.0D, e);
            double h = direction2.getDirection() == Direction.AxisDirection.POSITIVE ? 1.0D - g : g;
   ...
   private boolean wouldCollideAt(BlockPos pos) {
      ...
      return this.world.hasBlockCollision(this, box2, (state, posx) -> {
         return state.shouldSuffocate(this.world, posx);
      });
   }
   ...
   protected void autoJump(float dx, float dz) {
      if (this.shouldAutoJump()) {
         ...
         if (g <= 0.001F) {
            ...
            float h = f * vec2f.x;
            float i = f * vec2f.y;
            m = MathHelper.sin(this.getYaw() * 0.017453292F);
   ...
   public Vec3d method_30951(float f) {
      if (this.client.options.getPerspective().isFirstPerson()) {
         float g = MathHelper.lerp(f * 0.5F, this.getYaw(), this.prevYaw) * 0.017453292F;
         float h = MathHelper.lerp(f * 0.5F, this.getPitch(), this.prevPitch) * 0.017453292F;

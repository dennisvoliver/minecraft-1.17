...
public class TextRenderer {
   ...
   public void method_37296(OrderedText orderedText, float f, float g, int i, int j, Matrix4f matrix4f, VertexConsumerProvider vertexConsumerProvider, int k) {
         ...
         for(int n = -1; n <= 1; ++n) {
            if (m != 0 || n != 0) {
               ...
               orderedText.accept((lx, style, mx) -> {
                  ...
                  drawer.x = fs[0] + (float)m * glyph.getShadowOffset();
                  drawer.y = g + (float)n * glyph.getShadowOffset();
      ...
      TextRenderer.Drawer drawer2 = new TextRenderer.Drawer(vertexConsumerProvider, f, g, tweakTransparency(i), false, matrix4f, TextRenderer.TextLayerType.POLYGON_OFFSET, k);

...
public class NetworkUtils {
   ...
   private static String post(URL url, String content, boolean ignoreError, @Nullable Proxy proxy) {
      try {
         ...
         httpURLConnection.setRequestProperty("Content-Length", content.getBytes().length.makeConcatWithConstants<invokedynamic>(content.getBytes().length));
         ...
         if (!ignoreError) {
            LOGGER.error((String)"Could not post to {}", (Object)url, (Object)var9);
         }
   ...
   public static CompletableFuture<?> downloadResourcePack(File file, String url, Map<String, String> headers, int maxFileSize, @Nullable ProgressListener progressListener, Proxy proxy) {
      return CompletableFuture.supplyAsync(() -> {
         ...
         try {
            ...
            if (progressListener != null) {
               progressListener.setTask(new TranslatableText("resourcepack.progress", new Object[]{String.format(Locale.ROOT, "%.2f", g / 1000.0F / 1000.0F)}));
            }

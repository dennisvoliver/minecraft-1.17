...
public class WorldRenderer implements SynchronousResourceReloader, AutoCloseable {
   ...
   public void tickRainSplashing(Camera camera) {
      ...
      if (!(f <= 0.0F)) {
         ...
         for(int j = 0; j < i; ++j) {
            ...
            if (blockPos3.getY() > worldView.getBottomY() && blockPos3.getY() <= blockPos.getY() + 10 && blockPos3.getY() >= blockPos.getY() - 10 && biome.getPrecipitation() == Biome.Precipitation.RAIN && biome.getTemperature(blockPos3) >= 0.15F) {
               blockPos2 = blockPos3;
   ...
   private void loadTransparencyShader() {
      ...
      try {
         ...
         if (this.client.getResourcePackManager().getEnabledNames().size() > 1) {
            ...
            this.client.onResourceReloadFailure(shaderException, text2);
         } else {
   ...
   public String getEntitiesDebugString() {
      ...
      return "E: " + var10000 + "/" + this.world.getRegularEntityCount() + ", B: " + this.blockEntityCount;
   }
            ...
            if (!bl2 || !chunkInfo.canCull(direction.getOpposite())) {
               ...
               if (builtChunk4 != null && builtChunk4.shouldBuild()) {
                  ...
                  if (!builtChunk4.setRebuildFrame(i)) {
                     ...
                     chunkInfo2 = new WorldRenderer.ChunkInfo(builtChunk4, direction, chunkInfo.propagationLevel + 1);
      ...
      if (this.client.debugChunkInfo || this.client.debugChunkOcclusion) {
         ...
         while(var10.hasNext()) {
            ...
            if (this.client.debugChunkInfo) {
               ...
               n = chunkInfo.propagationLevel == 0 ? 0 : MathHelper.hsvToRgb((float)chunkInfo.propagationLevel / 50.0F, 0.9F, 0.9F);
               int j = n >> 16 & 255;
               k = n >> 8 & 255;
               l = n & 255;
               ...
               for(int m = 0; m < DIRECTIONS.length; ++m) {
                  if (chunkInfo.hasDirection(m)) {
                     ...
                     bufferBuilder.vertex((double)(8 - 16 * direction.getOffsetX()), (double)(8 - 16 * direction.getOffsetY()), (double)(8 - 16 * direction.getOffsetZ())).color(j, k, l, 255).next();
                  }
               }
            ...
            if (this.client.debugChunkOcclusion && !builtChunk.getData().isEmpty()) {
               ...
               while(true) {
                  ...
                  Direction direction2 = var26[l];
                  Direction[] var30 = DIRECTIONS;
                  int var21 = var30.length;

                  for(int var22 = 0; var22 < var21; ++var22) {
                     Direction direction3 = var30[var22];
      ...
      if (!(camera.getPos().x < worldBorder.getBoundEast() - d) || !(camera.getPos().x > worldBorder.getBoundWest() + d) || !(camera.getPos().z < worldBorder.getBoundSouth() - d) || !(camera.getPos().z > worldBorder.getBoundNorth() + d)) {
         ...
         if (f > worldBorder.getBoundEast() - d) {
            ...
            for(af = q; af < r; ae += 0.5F) {
               ...
               ah = (float)ag * 0.5F;
               bufferBuilder.vertex(worldBorder.getBoundEast() - f, -h, af - g).texture(m - ae, m + p).next();
               bufferBuilder.vertex(worldBorder.getBoundEast() - f, -h, af + ag - g).texture(m - (ah + ae), m + p).next();
               bufferBuilder.vertex(worldBorder.getBoundEast() - f, h, af + ag - g).texture(m - (ah + ae), m + 0.0F).next();
               bufferBuilder.vertex(worldBorder.getBoundEast() - f, h, af - g).texture(m - ae, m + 0.0F).next();
               ++af;
            }
         }
         ...
         if (f < worldBorder.getBoundWest() + d) {
            ...
            for(af = q; af < r; ae += 0.5F) {
               ag = Math.min(1.0D, r - af);
               ah = (float)ag * 0.5F;
               bufferBuilder.vertex(worldBorder.getBoundWest() - f, -h, af - g).texture(m + ae, m + p).next();
               bufferBuilder.vertex(worldBorder.getBoundWest() - f, -h, af + ag - g).texture(m + ah + ae, m + p).next();
               bufferBuilder.vertex(worldBorder.getBoundWest() - f, h, af + ag - g).texture(m + ah + ae, m + 0.0F).next();
               bufferBuilder.vertex(worldBorder.getBoundWest() - f, h, af - g).texture(m + ae, m + 0.0F).next();
               ++af;
            }
         }
         ...
         if (g > worldBorder.getBoundSouth() - d) {
            ...
            for(af = q; af < r; ae += 0.5F) {
               ag = Math.min(1.0D, r - af);
               ah = (float)ag * 0.5F;
               bufferBuilder.vertex(af - f, -h, worldBorder.getBoundSouth() - g).texture(m + ae, m + p).next();
               bufferBuilder.vertex(af + ag - f, -h, worldBorder.getBoundSouth() - g).texture(m + ah + ae, m + p).next();
               bufferBuilder.vertex(af + ag - f, h, worldBorder.getBoundSouth() - g).texture(m + ah + ae, m + 0.0F).next();
               bufferBuilder.vertex(af - f, h, worldBorder.getBoundSouth() - g).texture(m + ae, m + 0.0F).next();
               ++af;
            }
         }
   ...
   public void processWorldEvent(PlayerEntity source, int eventId, BlockPos pos, int data) {
      ...
      case 3002:
         if (data >= 0 && data < Direction.Axis.VALUES.length) {
            ...
            ParticleUtil.spawnParticle(this.world, pos, (ParticleEffect)ParticleTypes.ELECTRIC_SPARK, (UniformIntProvider)UniformIntProvider.create(3, 5));
         }
         break;
      case 3003:
         ParticleUtil.spawnParticle(this.world, pos, (ParticleEffect)ParticleTypes.WAX_ON, (UniformIntProvider)UniformIntProvider.create(3, 5));
         this.world.playSound(pos, SoundEvents.ITEM_HONEYCOMB_WAX_ON, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
         break;
      case 3004:
         ParticleUtil.spawnParticle(this.world, pos, (ParticleEffect)ParticleTypes.WAX_OFF, (UniformIntProvider)UniformIntProvider.create(3, 5));
         break;
      case 3005:
         ParticleUtil.spawnParticle(this.world, pos, (ParticleEffect)ParticleTypes.SCRAPE, (UniformIntProvider)UniformIntProvider.create(3, 5));
      }

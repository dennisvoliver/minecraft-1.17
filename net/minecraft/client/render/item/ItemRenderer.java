...
public class ItemRenderer implements SynchronousResourceReloader {
   ...
   public BakedModel getHeldItemModel(ItemStack stack, @Nullable World world, @Nullable LivingEntity entity, int seed) {
      ...
      if (stack.isOf(Items.TRIDENT)) {
         bakedModel3 = this.models.getModelManager().getModel(new ModelIdentifier("minecraft:trident_in_hand#inventory"));
      } else if (stack.isOf(Items.SPYGLASS)) {
         bakedModel3 = this.models.getModelManager().getModel(new ModelIdentifier("minecraft:spyglass_in_hand#inventory"));
      } else {
         bakedModel3 = this.models.getModel(stack);
      }

      ClientWorld clientWorld = world instanceof ClientWorld ? (ClientWorld)world : null;
      BakedModel bakedModel4 = bakedModel3.getOverrides().apply(bakedModel3, stack, clientWorld, entity, seed);
   ...
   public void renderGuiItemIcon(ItemStack stack, int x, int y) {
      this.renderGuiItemModel(stack, x, y, this.getHeldItemModel(stack, (World)null, (LivingEntity)null, 0));
   }
   ...
   public void renderInGui(ItemStack stack, int x, int y) {
      this.method_36543((LivingEntity)null, stack, x, y, 0);
   }
   ...
   private void innerRenderInGui(@Nullable LivingEntity entity, ItemStack itemStack, int x, int y, int i, int j) {
      if (!itemStack.isEmpty()) {
         BakedModel bakedModel = this.getHeldItemModel(itemStack, (World)null, entity, i);

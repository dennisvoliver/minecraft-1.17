...
public class WeightedBakedModel implements BakedModel {
   ...
   public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction face, Random random) {
      return (List)Weighting.getAt(this.models, Math.abs((int)random.nextLong()) % this.totalWeight).map((present) -> {
         return ((BakedModel)present.getData()).getQuads(state, face, random);

...
public class RabbitEntityModel<T extends RabbitEntity> extends EntityModel<T> {
   ...
   public void render(MatrixStack matrices, VertexConsumer vertices, int light, int overlay, float red, float green, float blue, float alpha) {
      if (this.child) {
         ...
         ImmutableList.of(this.leftHindLeg, this.rightHindLeg, this.leftHaunch, this.rightHaunch, this.body, this.leftFrontLeg, this.rightFrontLeg, this.head, this.rightEar, this.leftEar, this.tail, this.nose).forEach((part) -> {
            part.render(matrices, vertices, light, overlay, red, green, blue, alpha);
         });
         matrices.pop();
      }
   ...
   public void setAngles(T rabbitEntity, float f, float g, float h, float i, float j) {
      ...
      this.jumpProgress = MathHelper.sin(rabbitEntity.getJumpProgress(k) * 3.1415927F);
   ...
   public void animateModel(T rabbitEntity, float f, float g, float h) {
      ...
      this.jumpProgress = MathHelper.sin(rabbitEntity.getJumpProgress(h) * 3.1415927F);
   }
}

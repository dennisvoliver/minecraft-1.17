...
public class EndermiteEntityModel<T extends Entity> extends SinglePartEntityModel<T> {
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
      for(int i = 0; i < this.bodySegments.length; ++i) {
         this.bodySegments[i].yaw = MathHelper.cos(animationProgress * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.01F * (float)(1 + Math.abs(i - 2));
         this.bodySegments[i].pivotX = MathHelper.sin(animationProgress * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.1F * (float)Math.abs(i - 2);
      }

...
public class ShulkerEntityRenderer extends MobEntityRenderer<ShulkerEntity, ShulkerEntityModel<ShulkerEntity>> {
   ...
   public boolean shouldRender(ShulkerEntity shulkerEntity, Frustum frustum, double d, double e, double f) {
      return super.shouldRender((MobEntity)shulkerEntity, frustum, d, e, f) ? true : shulkerEntity.method_33352(0.0F).filter((vec3d) -> {
         ...
         return frustum.isVisible((new Box(vec3d.x, vec3d.y + (double)f, vec3d.z, vec3d2.x, vec3d2.y + (double)f, vec3d2.z)).expand((double)g, (double)f, (double)g));
      }).isPresent();
   }

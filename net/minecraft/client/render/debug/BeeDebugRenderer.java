...
public class BeeDebugRenderer implements DebugRenderer.Renderer {
   ...
   private void removeInvalidBees() {
      this.bees.entrySet().removeIf((bee) -> {
         return this.client.world.getEntityById(((BeeDebugRenderer.Bee)bee.getValue()).entityId) == null;
      });
   }
   ...
   private void removeOutdatedHives() {
      ...
      this.hives.entrySet().removeIf((hive) -> {
         return ((BeeDebugRenderer.Hive)hive.getValue()).time < l;
      });
   }
   ...
   private void render() {
      ...
      this.getBeesByHive().forEach((hivePos, bees) -> {
         if (blockPos.isWithinDistance(hivePos, 30.0D)) {
   ...
   private void drawHiveBees(BlockPos pos, List<String> bees) {
      float f = 0.05F;
      drawBox(pos, 0.05F, 0.2F, 0.2F, 1.0F, 0.3F);
      drawString(bees.makeConcatWithConstants<invokedynamic>(bees), (BlockPos)pos, 0, -256);
   ...
   private void drawHiveInfo(BeeDebugRenderer.Hive hive, Collection<UUID> blacklistingBees) {
      ...
      int var4 = hive.honeyLevel;
      drawString("Honey: " + var4, (BeeDebugRenderer.Hive)hive, i++, -23296);

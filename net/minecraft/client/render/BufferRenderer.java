   ...
   public static void draw(BufferBuilder bufferBuilder) {
      if (!RenderSystem.isOnRenderThreadOrInit()) {
         RenderSystem.recordRenderCall(() -> {
            ...
            draw((ByteBuffer)pair.getSecond(), drawArrayParameters.getMode(), drawArrayParameters.getVertexFormat(), drawArrayParameters.getCount(), drawArrayParameters.getElementFormat(), drawArrayParameters.getVertexCount(), drawArrayParameters.isTextured());
         });
      } else {
         Pair<BufferBuilder.DrawArrayParameters, ByteBuffer> pair = bufferBuilder.popData();
         BufferBuilder.DrawArrayParameters drawArrayParameters = (BufferBuilder.DrawArrayParameters)pair.getFirst();
   ...
   private static void draw(ByteBuffer buffer, VertexFormat.DrawMode drawMode, VertexFormat vertexFormat, int count, VertexFormat.IntType elementFormat, int vertexCount, boolean textured) {
      ...
      if (count > 0) {
         ...
         if (textured) {
            ...
            n = indexBuffer.getId();
            if (n != currentElementBuffer) {
               GlStateManager._glBindBuffer(34963, n);
               currentElementBuffer = n;
            }

            m = indexBuffer.getElementFormat().count;
         } else {
         ...
         shader.upload();
         GlStateManager._drawElements(drawMode.mode, vertexCount, m, 0L);
         shader.bind();
         buffer.position(0);
      }
   }

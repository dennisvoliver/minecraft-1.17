...
public class Keyboard {
   ...
   private void addDebugMessage(Formatting formatting, Text text) {
      this.client.inGameHud.getChatHud().addMessage((new LiteralText("")).append((new TranslatableText("debug.prefix")).formatted(new Formatting[]{formatting, Formatting.BOLD})).append(" ").append(text));
   }
      ...
      if (this.debugCrashStartTime > 0L && this.debugCrashStartTime < Util.getMeasuringTimeMs() - 100L) {
         ...
         case 78:
            if (!this.client.player.hasPermissionLevel(2)) {
               ...
               ClientPlayerEntity var10000 = this.client.player;
               GameMode var10001 = this.client.interactionManager.getPreviousGameMode();
               var10000.sendChatMessage("/gamemode " + ((GameMode)MoreObjects.firstNonNull(var10001, GameMode.CREATIVE)).getName());
            }
      ...
      if (window == this.client.getWindow().getHandle()) {
         ...
         if (action == 1 && (!(this.client.currentScreen instanceof ControlsOptionsScreen) || ((ControlsOptionsScreen)screen).time <= Util.getMeasuringTimeMs() - 20L)) {
            ...
            if (this.client.options.keyScreenshot.matchesKey(key, scancode)) {
               ...
               ScreenshotRecorder.saveScreenshot(this.client.runDirectory, this.client.getWindow().getFramebufferWidth(), this.client.getWindow().getFramebufferHeight(), this.client.getFramebuffer(), (message) -> {
                  this.client.execute(() -> {
         ...
         if (NarratorManager.INSTANCE.isActive()) {
            ...
            if (action != 0 && key == GLFW.GLFW_KEY_B && Screen.hasControlDown() && bl) {
               bl3 = this.client.options.narrator == NarratorMode.OFF;
            ...
            if (action == 0) {
               ...
               if (this.client.currentScreen == null) {
                  if (key == GLFW.GLFW_KEY_ESCAPE) {
                     boolean bl4 = InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), GLFW.GLFW_KEY_F3);
                     this.client.openPauseMenu(bl4);
                  }

                  bl3 = InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), GLFW.GLFW_KEY_F3) && this.processF3(key);

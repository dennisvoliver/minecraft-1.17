...
public class WindowFramebuffer extends Framebuffer {
   ...
   private WindowFramebuffer.Size findSuitableSize(int width, int height) {
      ...
      do {
         ...
         size = (WindowFramebuffer.Size)var4.next();
      ...
      } while(attachment != WindowFramebuffer.Attachment.COLOR_DEPTH);
   ...
   static class Size {
      ...
      static List<WindowFramebuffer.Size> findCompatible(int width, int height) {
         ...
         return width > 0 && width <= i && height > 0 && height <= i ? ImmutableList.of(new WindowFramebuffer.Size(width, height), WindowFramebuffer.DEFAULT) : ImmutableList.of(WindowFramebuffer.DEFAULT);
      }

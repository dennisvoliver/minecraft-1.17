...
public class EnchantmentHelper {
      ...
      if (stack.isEmpty()) {
         ...
         for(int i = 0; i < nbtList.size(); ++i) {
            ...
            if (identifier2 != null && identifier2.equals(identifier)) {
               return MathHelper.clamp((int)nbtCompound.getInt("lvl"), (int)0, (int)255);
            }
         }
   ...
   public static List<EnchantmentLevelEntry> generateEnchantments(Random random, ItemStack stack, int level, boolean treasureAllowed) {
      ...
      if (i <= 0) {
         ...
         level = MathHelper.clamp((int)Math.round((float)level + (float)level * f), (int)1, (int)Integer.MAX_VALUE);

...
public class CommandManager {
   ...
   public CommandManager(CommandManager.RegistrationEnvironment environment) {
      ...
      this.dispatcher.findAmbiguities((parent, child, sibling, inputs) -> {
         LOGGER.warn((String)"Ambiguity between arguments {} and {} with inputs: {}", (Object)this.dispatcher.getPath(child), this.dispatcher.getPath(sibling), inputs);
      });
      ...
      try {
         try {
            ...
            if (SharedConstants.isDevelopment) {
               ...
               LOGGER.error((String)"'{}' threw an exception", (Object)command, (Object)var15);
            }
   ...
   public static void checkMissing() {
      ...
      Set<ArgumentType<?>> set2 = (Set)set.stream().filter((type) -> {
         return !ArgumentTypes.hasClass(type);

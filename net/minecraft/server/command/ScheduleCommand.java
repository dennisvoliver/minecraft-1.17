...
public class ScheduleCommand {
   ...
   private static final DynamicCommandExceptionType CLEARED_FAILURE_EXCEPTION = new DynamicCommandExceptionType((eventName) -> {
      return new TranslatableText("commands.schedule.cleared.failure", new Object[]{eventName});
   });
   private static final SuggestionProvider<ServerCommandSource> SUGGESTION_PROVIDER = (context, builder) -> {
      return CommandSource.suggestMatching((Iterable)((ServerCommandSource)context.getSource()).getMinecraftServer().getSaveProperties().getMainWorldProperties().getScheduledEvents().method_22592(), builder);
   };
   ...
   private static int clearEvent(ServerCommandSource source, String eventName) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         source.sendFeedback(new TranslatableText("commands.schedule.cleared.success", new Object[]{i, eventName}), true);
         return i;
      }
   }
}

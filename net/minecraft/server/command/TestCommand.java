...
public class TestCommand {
      ...
      if (x <= 48 && y <= 48 && z <= 48) {
         ...
         for(int i = 0; i < x; ++i) {
            for(int j = 0; j < z; ++j) {
               ...
               BlockStateArgument blockStateArgument = new BlockStateArgument(block.getDefaultState(), Collections.emptySet(), (NbtCompound)null);
   ...
   private static int executePos(ServerCommandSource source, String variableName) throws CommandSyntaxException {
      ...
      if (!optional.isPresent()) {
         ...
         int var10000 = blockPos2.getX();
         String string = var10000 + ", " + blockPos2.getY() + ", " + blockPos2.getZ();
   ...
   private static int executeClearAll(ServerCommandSource source, int radius) {
      ...
      TestUtil.clearTests(serverWorld, blockPos, TestManager.INSTANCE, MathHelper.clamp((int)radius, (int)0, (int)1024));
      return 1;
   }
   ...
   private static int executeRerunFailed(ServerCommandSource source, boolean requiredOnly, int rotationSteps, int sizeZ) {
      ...
      if (requiredOnly) {
         collection2 = (Collection)TestFunctions.getFailedTestFunctions().stream().filter(TestFunction::isRequired).collect(Collectors.toList());
      } else {
         collection2 = TestFunctions.getFailedTestFunctions();
      }
   ...
   private static void run(ServerCommandSource source, Collection<TestFunction> testFunctions, int rotationSteps, int i) {
      ...
      testSet.addListener((TestListener)(new TestCommand.Listener(serverWorld, testSet)));
      testSet.addListener((test) -> {
   ...
   private static void sendMessage(ServerWorld world, String message, Formatting formatting) {
      world.getPlayers((player) -> {
         ...
         player.sendSystemMessage(new LiteralText(formatting + message), Util.NIL_UUID);
      });
   }

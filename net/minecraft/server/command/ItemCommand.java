...
public class ItemCommand {
   static final Dynamic3CommandExceptionType NOT_A_CONTAINER_TARGET_EXCEPTION = new Dynamic3CommandExceptionType((x, y, z) -> {
      return new TranslatableText("commands.item.target.not_a_container", new Object[]{x, y, z});
   });
   private static final Dynamic3CommandExceptionType NOT_A_CONTAINER_SOURCE_EXCEPTION = new Dynamic3CommandExceptionType((x, y, z) -> {
      return new TranslatableText("commands.item.source.not_a_container", new Object[]{x, y, z});
   });
   static final DynamicCommandExceptionType NO_SUCH_SLOT_TARGET_EXCEPTION = new DynamicCommandExceptionType((slot) -> {
      return new TranslatableText("commands.item.target.no_such_slot", new Object[]{slot});
   });
   ...
   private static final DynamicCommandExceptionType NO_CHANGES_EXCEPTION = new DynamicCommandExceptionType((slot) -> {
      return new TranslatableText("commands.item.target.no_changes", new Object[]{slot});
   });
   private static final Dynamic2CommandExceptionType KNOWN_ITEM_EXCEPTION = new Dynamic2CommandExceptionType((itemName, slot) -> {
      return new TranslatableText("commands.item.target.no_changed.known_item", new Object[]{itemName, slot});
   });
   private static final SuggestionProvider<ServerCommandSource> MODIFIER_SUGGESTION_PROVIDER = (context, builder) -> {
      ...
      return CommandSource.suggestIdentifiers((Iterable)lootFunctionManager.getFunctionIds(), builder);
   };
   ...
   private static int executeBlockModify(ServerCommandSource source, BlockPos pos, int slot, LootFunction modifier) throws CommandSyntaxException {
      ...
      if (slot >= 0 && slot < inventory.size()) {
         ...
         source.sendFeedback(new TranslatableText("commands.item.block.set.success", new Object[]{pos.getX(), pos.getY(), pos.getZ(), itemStack.toHoverableText()}), true);
         return 1;
      } else {
   ...
   private static int executeEntityModify(ServerCommandSource source, Collection<? extends Entity> targets, int slot, LootFunction modifier) throws CommandSyntaxException {
      ...
      if (map.isEmpty()) {
         ...
         if (map.size() == 1) {
            ...
            source.sendFeedback(new TranslatableText("commands.item.entity.set.success.single", new Object[]{((Entity)entry.getKey()).getDisplayName(), ((ItemStack)entry.getValue()).toHoverableText()}), true);
         } else {
            source.sendFeedback(new TranslatableText("commands.item.entity.set.success.multiple", new Object[]{map.size()}), true);
         }
   ...
   private static int executeBlockReplace(ServerCommandSource source, BlockPos pos, int slot, ItemStack stack) throws CommandSyntaxException {
      ...
      if (slot >= 0 && slot < inventory.size()) {
         ...
         source.sendFeedback(new TranslatableText("commands.item.block.set.success", new Object[]{pos.getX(), pos.getY(), pos.getZ(), stack.toHoverableText()}), true);
         return 1;
      } else {
         throw NO_SUCH_SLOT_TARGET_EXCEPTION.create(slot);
      }
   ...
   private static int executeEntityReplace(ServerCommandSource source, Collection<? extends Entity> targets, int slot, ItemStack stack) throws CommandSyntaxException {
      ...
      if (list.isEmpty()) {
         ...
         if (list.size() == 1) {
            source.sendFeedback(new TranslatableText("commands.item.entity.set.success.single", new Object[]{((Entity)list.iterator().next()).getDisplayName(), stack.toHoverableText()}), true);
         } else {
            source.sendFeedback(new TranslatableText("commands.item.entity.set.success.multiple", new Object[]{list.size(), stack.toHoverableText()}), true);
         }

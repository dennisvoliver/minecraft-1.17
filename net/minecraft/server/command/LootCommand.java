...
public class LootCommand {
   public static final SuggestionProvider<ServerCommandSource> SUGGESTION_PROVIDER = (context, builder) -> {
      ...
      return CommandSource.suggestIdentifiers((Iterable)lootManager.getTableIds(), builder);
   };
   private static final DynamicCommandExceptionType NO_HELD_ITEMS_EXCEPTION = new DynamicCommandExceptionType((entityName) -> {
      return new TranslatableText("commands.drop.no_held_items", new Object[]{entityName});
   });
   private static final DynamicCommandExceptionType NO_LOOT_TABLE_EXCEPTION = new DynamicCommandExceptionType((entityName) -> {
      return new TranslatableText("commands.drop.no_loot_table", new Object[]{entityName});
   });
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)addTargetArguments((LiteralArgumentBuilder)CommandManager.literal("loot").requires((source) -> {
         ...
         })).then(CommandManager.argument("tool", ItemStackArgumentType.itemStack()).executes((context) -> {
   ...
   private static int executeMine(CommandContext<ServerCommandSource> context, BlockPos pos, ItemStack stack, LootCommand.Target constructor) throws CommandSyntaxException {
      ...
      return constructor.accept(context, list, (stacks) -> {
         sendDroppedFeedback(serverCommandSource, stacks, blockState.getBlock().getLootTableId());
      });
   }
   ...
   private static int executeKill(CommandContext<ServerCommandSource> context, Entity entity, LootCommand.Target constructor) throws CommandSyntaxException {
      if (!(entity instanceof LivingEntity)) {
         ...
         return constructor.accept(context, list, (stacks) -> {
            sendDroppedFeedback(serverCommandSource, stacks, identifier);
         });
      }
   }

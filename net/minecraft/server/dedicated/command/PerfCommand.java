...
public class PerfCommand {
   ...
   private static int executeStart(ServerCommandSource source) throws CommandSyntaxException {
      ...
      if (minecraftServer.isRunningMonitor()) {
         ...
         Consumer<Path> consumer2 = (path) -> {
            saveReport(source, path, minecraftServer);
         };
   ...
   private static void saveReport(ServerCommandSource source, Path tempProfilingFile, MinecraftServer server) {
      ...
      String string3;
      try {
         string3 = FileNameUtil.getNextUniqueName(ProfilerDumper.DEBUG_PROFILING_DIRECTORY, string, ".zip");
      ...
      ZipCompressor zipCompressor = new ZipCompressor(ProfilerDumper.DEBUG_PROFILING_DIRECTORY.resolve(string3));
      ...
      try {
         ...
         LOGGER.warn((String)"Failed to delete temporary profiling file {}", (Object)tempProfilingFile, (Object)var9);
      }

      source.sendFeedback(new TranslatableText("commands.perf.reportSaved", new Object[]{string3}), false);
   }
   ...
   private static void sendProfilingStoppedMessage(ServerCommandSource source, ProfileResult result) {
      ...
      source.sendFeedback(new TranslatableText("commands.perf.stopped", new Object[]{String.format(Locale.ROOT, "%.2f", d), i, String.format(Locale.ROOT, "%.2f", (double)i / d)}), false);
   }
}

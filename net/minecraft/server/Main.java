...
public class Main {
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         String string = (String)Optional.ofNullable((String)optionSet.valueOf((OptionSpec)optionSpec11)).orElse(serverPropertiesLoader.getPropertiesHandler().levelName);
         ...
         CompletableFuture completableFuture = ServerResourceManager.reload(resourcePackManager.createResourcePacks(), impl, CommandManager.RegistrationEnvironment.DEDICATED, serverPropertiesLoader.getPropertiesHandler().functionPermissionLevel, Util.getMainWorkerExecutor(), Runnable::run);

         ServerResourceManager serverResourceManager2;
         try {
            serverResourceManager2 = (ServerResourceManager)completableFuture.get();
         } catch (Exception var42) {
            LOGGER.warn((String)"Failed to load datapacks, can't proceed with server load. You can either fix your datapacks or reset to vanilla with --safeMode", (Throwable)var42);
            resourcePackManager.close();
            return;
         }

         serverResourceManager2.loadRegistryTags();
         RegistryOps<NbtElement> registryOps = RegistryOps.method_36574(NbtOps.INSTANCE, serverResourceManager2.getResourceManager(), impl);
         ...
         if (saveProperties == null) {
            ...
            if (optionSet.has((OptionSpec)optionSpec3)) {
               ...
               generatorOptions2 = GeneratorOptions.createDemo(impl);
            } else {
               ...
               generatorOptions2 = optionSet.has((OptionSpec)optionSpec4) ? serverPropertiesHandler.method_37371(impl).withBonusChest() : serverPropertiesHandler.method_37371(impl);
            }
         ...
         LOGGER.fatal((String)"Failed to start the minecraft server", (Throwable)var43);
      }

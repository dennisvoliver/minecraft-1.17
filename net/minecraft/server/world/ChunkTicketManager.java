   ...
   public boolean tick(ThreadedAnvilChunkStorage threadedAnvilChunkStorage) {
      ...
      if (!this.chunkHolders.isEmpty()) {
         this.chunkHolders.forEach((chunkHolderx) -> {
            chunkHolderx.tick(threadedAnvilChunkStorage, this.mainThreadExecutor);
         });
   ...
   private void method_34876(String string) {
      try {
         ...
         try {
            ...
            while(var3.hasNext()) {
               Entry<SortedArraySet<ChunkTicket<?>>> entry = (Entry)var3.next();
               ChunkPos chunkPos = new ChunkPos(entry.getLongKey());
               Iterator var6 = ((SortedArraySet)entry.getValue()).iterator();

               while(var6.hasNext()) {
                  ChunkTicket<?> chunkTicket = (ChunkTicket)var6.next();
                  int var10001 = chunkPos.x;
                  fileOutputStream.write((var10001 + "\t" + chunkPos.z + "\t" + chunkTicket.getType() + "\t" + chunkTicket.getLevel() + "\t\n").getBytes(StandardCharsets.UTF_8));
               }
            }
   ...
   private class DistanceFromNearestPlayerTracker extends ChunkPosDistanceLevelPropagator {
      ...
      private void method_34878(String string) {
         try {
            ...
            ChunkTicketManager.LOGGER.error((Object)var9);
         }

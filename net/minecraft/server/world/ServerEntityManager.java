...
public class ServerEntityManager<T extends EntityLike> implements AutoCloseable {
   ...
   private boolean addEntityUuid(T entity) {
      if (!this.entityUuids.add(entity.getUuid())) {
         LOGGER.warn((String)"UUID of added entity already exists: {}", (Object)entity);
         return false;
      } else {
         return true;
      }
   ...
   public void updateTrackingStatus(ChunkPos chunkPos, EntityTrackingStatus trackingStatus) {
      ...
      this.cache.getTrackingSections(l).forEach((group) -> {
         ...
         if (bl3 && !bl4) {
            group.stream().filter((entityLike) -> {
   ...
   private boolean trySave(long chunkPos, Consumer<T> action) {
      ...
      if (status == ServerEntityManager.Status.PENDING) {
         ...
         List<T> list = (List)this.cache.getTrackingSections(chunkPos).flatMap((entityTrackingSection) -> {
            return entityTrackingSection.stream().filter(EntityLike::shouldSave);
   ...
   private void scheduleRead(long chunkPos) {
      ...
      var10000.thenAccept(var10001::add).exceptionally((throwable) -> {
         LOGGER.error((String)"Failed to read chunk {}", (Object)chunkPos2, (Object)throwable);
         return null;
      });
   }

   private boolean unload(long chunkPos) {
      boolean bl = this.trySave(chunkPos, (entityLike) -> {
   ...
   private LongSet getLoadedChunks() {
      ...
      while(var2.hasNext()) {
         Entry<ServerEntityManager.Status> entry = (Entry)var2.next();
   ...
   class Listener implements EntityChangeListener {
      ...
      public void updateEntityPosition() {
         ...
         if (l != this.sectionPos) {
            ...
            if (!this.section.remove(this.entity)) {
               ServerEntityManager.LOGGER.warn((String)"Entity {} wasn't found in section {} (moving to {})", (Object)this.entity, ChunkSectionPos.from(this.sectionPos), l);
            }
      ...
      public void remove(Entity.RemovalReason reason) {
         if (!this.section.remove(this.entity)) {
            ServerEntityManager.LOGGER.warn((String)"Entity {} wasn't found in section {} (destroying due to {})", (Object)this.entity, ChunkSectionPos.from(this.sectionPos), reason);
         }

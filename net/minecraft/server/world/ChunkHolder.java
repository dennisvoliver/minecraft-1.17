...
public class ChunkHolder {
   ...
   @Nullable
   private final AtomicStack<ChunkHolder.MultithreadAction> actionStack;
            ...
            if (shortSet != null) {
               ...
               if (shortSet.size() == 1) {
                  ...
                  chunkDeltaUpdateS2CPacket.visitUpdates((pos, state) -> {
                     this.tryUpdateBlockEntityAt(world, pos, state);
                  });
               }
   ...
   private void method_31409(ThreadedAnvilChunkStorage threadedAnvilChunkStorage, CompletableFuture<Either<WorldChunk, ChunkHolder.Unloaded>> completableFuture, Executor executor, ChunkHolder.LevelType levelType) {
   ...
   protected void tick(ThreadedAnvilChunkStorage chunkStorage, Executor executor) {
      ...
      if (bl) {
         ...
         for(int i = bl2 ? chunkStatus2.getIndex() + 1 : 0; i <= chunkStatus.getIndex(); ++i) {
            completableFuture2 = (CompletableFuture)this.futuresByStatus.get(i);
      ...
      if (bl3 && !bl4) {
         completableFuture2 = this.accessibleFuture;
         this.accessibleFuture = UNLOADED_WORLD_CHUNK_FUTURE;
         this.combineSavingFuture(completableFuture2.thenApply((eitherx) -> {
   ...
   public static ChunkHolder.LevelType getLevelType(int distance) {
      return LEVEL_TYPES[MathHelper.clamp((int)(33 - distance + 1), (int)0, (int)(LEVEL_TYPES.length - 1))];
   }

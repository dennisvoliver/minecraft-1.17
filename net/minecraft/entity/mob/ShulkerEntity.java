...
public class ShulkerEntity extends GolemEntity implements Monster {
   ...
   private void moveEntities() {
      ...
      float h = f - g;
      if (!(h <= 0.0F)) {
         List<Entity> list = this.world.getOtherEntities(this, method_33347(direction, g, f).offset(this.getX() - 0.5D, this.getY(), this.getZ() - 0.5D), EntityPredicates.EXCEPT_SPECTATOR.and((entityx) -> {
   ...
   public double getHeightOffset() {
      ...
      return entityType != EntityType.BOAT && entityType != EntityType.MINECART ? super.getHeightOffset() : 0.1875D - this.getVehicle().getMountedHeightOffset();
   }
   ...
   boolean canStay(BlockPos pos, Direction direction) {
      if (this.method_33351(pos)) {
         ...
         if (!this.world.isDirectionSolid(pos.offset(direction), this, direction2)) {
            ...
            Box box = method_33346(direction2, 1.0F).offset(pos).contract(1.0E-6D);
   ...
   protected boolean tryTeleport() {
      if (!this.isAiDisabled() && this.isAlive()) {
         ...
         for(int i = 0; i < 5; ++i) {
            ...
            if (blockPos2.getY() > this.world.getBottomY() && this.world.isAir(blockPos2) && this.world.getWorldBorder().contains(blockPos2) && this.world.isSpaceEmpty(this, (new Box(blockPos2)).contract(1.0E-6D))) {
   ...
   private void spawnNewShulker() {
      ...
      if (!this.isClosed() && this.tryTeleport()) {
         ...
         if (!(this.world.random.nextFloat() < f)) {
            ShulkerEntity shulkerEntity = (ShulkerEntity)EntityType.SHULKER.create(this.world);
   ...
   public Optional<Vec3d> method_33352(float f) {
      if (this.prevAttachedBlock != null && this.teleportLerpTimer > 0) {
         double d = (double)((float)this.teleportLerpTimer - f) / 6.0D;
         d *= d;
         BlockPos blockPos = this.getBlockPos();
   ...
   private class class_6376 extends LookControl {
      ...
      protected Optional<Float> getTargetYaw() {
         ...
         return !(Math.abs(g) > 1.0E-5F) && !(Math.abs(h) > 1.0E-5F) ? Optional.empty() : Optional.of((float)(MathHelper.atan2((double)(-g), (double)h) * 57.2957763671875D));
      }

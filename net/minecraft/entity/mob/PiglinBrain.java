...
public class PiglinBrain {
   ...
   private static final UniformIntProvider HUNT_MEMORY_DURATION;
   ...
   private static void addCoreActivities(Brain<PiglinEntity> piglin) {
      piglin.setTaskList(Activity.CORE, 0, ImmutableList.of(new LookAroundTask(45, 90), new WanderAroundTask(), new OpenDoorsTask(), goToNemesisTask(), makeGoToZombifiedPiglinTask(), new RemoveOffHandItemTask(), new AdmireItemTask(120), new DefeatTargetTask(300, PiglinBrain::isHuntingTarget), new ForgetAngryAtTargetTask()));
   }
   ...
   private static MemoryTransferTask<PiglinEntity, LivingEntity> makeGoToZombifiedPiglinTask() {
      return new MemoryTransferTask(PiglinBrain::getNearestZombifiedPiglin, MemoryModuleType.NEAREST_VISIBLE_ZOMBIFIED, MemoryModuleType.AVOID_TARGET, GO_TO_ZOMBIFIED_MEMORY_DURATION);
   }
   ...
   protected static void loot(PiglinEntity piglin, ItemEntity drop) {
      ...
      } else if (isFood(itemStack2) && !hasAteRecently(piglin)) {
      ...
      if (getNearestZombifiedPiglin(piglin)) {
         ...
         if (optional.isPresent() && Sensor.method_36982(piglin, (LivingEntity)optional.get())) {
            ...
            if (optional3.isPresent()) {
               ...
               return optional4.isPresent() && Sensor.method_36982(piglin, (LivingEntity)optional4.get()) ? optional4 : Optional.empty();
            }
         }
      }
   }
   ...
   public static Optional<LivingEntity> getAvoiding(PiglinEntity piglin) {
      return piglin.getBrain().hasMemoryModule(MemoryModuleType.AVOID_TARGET) ? piglin.getBrain().getOptionalMemory(MemoryModuleType.AVOID_TARGET) : Optional.empty();
   }
   ...
   static {
      ...
      GO_TO_ZOMBIFIED_MEMORY_DURATION = Durations.betweenSeconds(5, 7);
      GO_TO_NEMESIS_MEMORY_DURATION = Durations.betweenSeconds(5, 7);
   }
}

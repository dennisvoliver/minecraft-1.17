...
public class ElderGuardianEntity extends GuardianEntity {
   ...
   protected void mobTick() {
      ...
      if ((this.age + this.getId()) % 1200 == 0) {
         ...
         List<ServerPlayerEntity> list = ((ServerWorld)this.world).getPlayers((player) -> {
            return this.squaredDistanceTo(player) < 2500.0D && player.interactionManager.isSurvivalLike();
         });
         int j = true;
         int k = true;
         int l = true;
         ...
         while(true) {
            ...
            serverPlayerEntity.networkHandler.sendPacket(new GameStateChangeS2CPacket(GameStateChangeS2CPacket.ELDER_GUARDIAN_EFFECT, this.isSilent() ? GameStateChangeS2CPacket.DEMO_OPEN_SCREEN : 1.0F));

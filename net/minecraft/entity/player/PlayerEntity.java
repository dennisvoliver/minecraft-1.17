...
public abstract class PlayerEntity extends LivingEntity {
         ...
         if (!target.handleAttack(this)) {
            ...
            if (f > 0.0F || h > 0.0F) {
               ...
               if (bl6) {
                  if (j > 0) {
                     if (target instanceof LivingEntity) {
                        ((LivingEntity)target).takeKnockback((double)((float)j * 0.5F), (double)MathHelper.sin(this.getYaw() * 0.017453292F), (double)(-MathHelper.cos(this.getYaw() * 0.017453292F)));
                     } else {
   ...
   public void increaseTravelMotionStats(double dx, double dy, double dz) {
      if (!this.hasVehicle()) {
         int l;
         if (this.isSwimming()) {
            l = Math.round((float)Math.sqrt(dx * dx + dy * dy + dz * dz) * 100.0F);
            if (l > 0) {
            ...
            l = Math.round((float)Math.sqrt(dx * dx + dz * dz) * 100.0F);
            if (l > 0) {
   ...
   public void addExperience(int experience) {
      ...
      this.totalExperience = MathHelper.clamp((int)(this.totalExperience + experience), (int)0, (int)Integer.MAX_VALUE);
   ...
   public Vec3d method_30951(float f) {
      ...
      float g = MathHelper.lerp(f * 0.5F, this.getPitch(), this.prevPitch) * 0.017453292F;
      ...
      if (!this.isFallFlying() && !this.isUsingRiptide()) {
         if (this.isInSwimmingPose()) {
            return this.getLerpedPos(f).add((new Vec3d(d, 0.2D, -0.15D)).rotateX(-g).rotateY(-h));
         } else {
            double n = this.getBoundingBox().getYLength() - 1.0D;
            e = this.isInSneakingPose() ? -0.2D : 0.07D;
            return this.getLerpedPos(f).add((new Vec3d(d, n, e)).rotateY(-h));
         }
      } else {
         ...
         return this.getLerpedPos(f).add((new Vec3d(d, -0.11D, 0.85D)).rotateZ(-m).rotateX(-g).rotateY(-h));
      }
   }

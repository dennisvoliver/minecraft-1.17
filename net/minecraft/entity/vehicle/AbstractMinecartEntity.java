...
public abstract class AbstractMinecartEntity extends Entity {
   ...
   public void tick() {
      ...
      if (this.world.isClient) {
         if (this.clientInterpolationSteps > 0) {
            h = this.getX() + (this.clientX - this.getX()) / (double)this.clientInterpolationSteps;
         ...
         if (!this.hasNoGravity()) {
            h = this.isTouchingWater() ? -0.005D : -0.04D;
            this.setVelocity(this.getVelocity().add(0.0D, h, 0.0D));
         }
         ...
         if (l * l + m * m > 0.001D) {
            this.setYaw((float)(MathHelper.atan2(m, l) * 180.0D / 3.141592653589793D));
            if (this.yawFlipped) {
         ...
         this.setRotation(this.getYaw(), this.getPitch());
         if (this.getMinecartType() == AbstractMinecartEntity.Type.RIDEABLE && this.getVelocity().horizontalLengthSquared() > 0.01D) {
   ...
   protected void moveOnRail(BlockPos pos, BlockState state) {
      ...
      switch(railShape) {
      case ASCENDING_EAST:
         this.setVelocity(vec3d2.add(-g, 0.0D, 0.0D));
         ++e;
         break;
      case ASCENDING_WEST:
         this.setVelocity(vec3d2.add(g, 0.0D, 0.0D));
      ...
      case ASCENDING_NORTH:
         this.setVelocity(vec3d2.add(0.0D, 0.0D, g));
         ++e;
         break;
      ...
      double l = Math.min(2.0D, vec3d2.horizontalLength());
            ...
            if (!this.hasPassenger(entity)) {
               ...
               if (f >= 9.999999747378752E-5D) {
                  ...
                  if (entity instanceof AbstractMinecartEntity) {
                     ...
                     Vec3d vec3d2 = (new Vec3d((double)MathHelper.cos(this.getYaw() * 0.017453292F), 0.0D, (double)MathHelper.sin(this.getYaw() * 0.017453292F))).normalize();

...
public class TemptationsSensor extends Sensor<PathAwareEntity> {
   ...
   protected void sense(ServerWorld serverWorld, PathAwareEntity pathAwareEntity) {
      ...
      Stream var10000 = serverWorld.getPlayers().stream().filter(EntityPredicates.EXCEPT_SPECTATOR).filter((player) -> {
         return TEMPTER_PREDICATE.test(pathAwareEntity, player);
      }).filter((player) -> {
         return pathAwareEntity.isInRange(player, 10.0D);
      }).filter(this::test);
      Objects.requireNonNull(pathAwareEntity);
      List<PlayerEntity> list = (List)var10000.sorted(Comparator.comparingDouble(pathAwareEntity::squaredDistanceTo)).collect(Collectors.toList());
      if (!list.isEmpty()) {

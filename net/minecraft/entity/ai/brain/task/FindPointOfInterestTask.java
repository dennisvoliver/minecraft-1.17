...
public class FindPointOfInterestTask extends Task<PathAwareEntity> {
   ...
   protected void run(ServerWorld serverWorld, PathAwareEntity pathAwareEntity, long l) {
      ...
      this.foundPositionsToExpiry.long2ObjectEntrySet().removeIf((entry) -> {
         return !((FindPointOfInterestTask.RetryMarker)entry.getValue()).isAttempting(l);
      });
      ...
      Set<BlockPos> set = (Set)pointOfInterestStorage.getSortedPositions(this.poiType.getCompletionCondition(), predicate, pathAwareEntity.getBlockPos(), 48, PointOfInterestStorage.OccupationStatus.HAS_SPACE).limit(5L).collect(Collectors.toSet());

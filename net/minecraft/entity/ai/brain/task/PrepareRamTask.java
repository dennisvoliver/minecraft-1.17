...
public class PrepareRamTask<E extends PathAwareEntity> extends Task<E> {
   ...
   protected void run(ServerWorld serverWorld, PathAwareEntity pathAwareEntity, long l) {
      ...
      brain.getOptionalMemory(MemoryModuleType.VISIBLE_MOBS).flatMap((mobs) -> {
         return mobs.stream().filter((mob) -> {
            return this.targetPredicate.test(pathAwareEntity, mob);
         }).findFirst();
      }).ifPresent((mob) -> {
   ...
   protected void keepRunning(ServerWorld serverWorld, E pathAwareEntity, long l) {
      if (this.ram.isPresent()) {
         pathAwareEntity.getBrain().remember(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget(((PrepareRamTask.Ram)this.ram.get()).getStart(), this.speed, 0)));
         pathAwareEntity.getBrain().remember(MemoryModuleType.LOOK_TARGET, (Object)(new EntityLookTarget(((PrepareRamTask.Ram)this.ram.get()).getEntity(), true)));
         ...
         if (bl) {
            ...
            if (blockPos.equals(((PrepareRamTask.Ram)this.ram.get()).getStart())) {
               ...
               if (l - (Long)this.prepareStartTime.get() >= (long)this.prepareTime) {
                  pathAwareEntity.getBrain().remember(MemoryModuleType.RAM_TARGET, (Object)this.calculateRamTarget(blockPos, ((PrepareRamTask.Ram)this.ram.get()).getEnd()));
                  serverWorld.playSoundFromEntity((PlayerEntity)null, pathAwareEntity, (SoundEvent)this.soundFactory.apply(pathAwareEntity), SoundCategory.HOSTILE, 1.0F, pathAwareEntity.getSoundPitch());
   ...
   private Vec3d calculateRamTarget(BlockPos start, BlockPos end) {
      ...
      return Vec3d.ofBottomCenter(end).add(e, 0.0D, f);
   }
   ...
   private void findRam(PathAwareEntity entity, LivingEntity target) {
      ...
      this.ram = this.findRamStart(entity, target).map((start) -> {
         return new PrepareRamTask.Ram(start, target.getBlockPos(), target);
      });
   }

...
public class OpenDoorsTask extends Task<LivingEntity> {
   ...
   private static boolean hasOtherMobReachedDoor(ServerWorld world, LivingEntity entity, BlockPos pos) {
      ...
      return !brain.hasMemoryModule(MemoryModuleType.MOBS) ? false : ((List)brain.getOptionalMemory(MemoryModuleType.MOBS).get()).stream().filter((livingEntity2) -> {
         ...
         return pos.isWithinDistance(livingEntity.getPos(), 2.0D);
      }).anyMatch((livingEntity) -> {
         return hasReached(world, livingEntity, pos);
      });
   }
   ...
   private static boolean cannotReachDoor(ServerWorld world, LivingEntity entity, GlobalPos doorPos) {
      return doorPos.getDimension() != world.getRegistryKey() || !doorPos.getPos().isWithinDistance(entity.getPos(), 2.0D);
   }

...
public class LongJumpTask<E extends MobEntity> extends Task<E> {
   ...
   protected boolean shouldKeepRunning(ServerWorld serverWorld, MobEntity mobEntity, long l) {
      boolean bl = this.lastPos.isPresent() && ((Vec3d)this.lastPos.get()).equals(mobEntity.getPos()) && this.cooldown > 0 && (this.lastTarget.isPresent() || !this.targets.isEmpty());
   ...
   protected void run(ServerWorld serverWorld, MobEntity mobEntity, long l) {
      ...
      while(true) {
         ...
         do {
            ...
            d = blockPos2.getSquaredDistance(blockPos);
         } while(i == blockPos2.getX() && k == blockPos2.getZ());

         if (entityNavigation.isValidPosition(blockPos2) && mobEntity.getPathfindingPenalty(LandPathNodeMaker.getLandNodeType(mobEntity.world, blockPos2.mutableCopy())) == 0.0F) {
            Optional<Vec3d> optional = this.getRammingVelocity(mobEntity, Vec3d.ofCenter(blockPos2));
            optional.ifPresent((vel) -> {
               this.targets.add(new LongJumpTask.Target(new BlockPos(blockPos2), vel, MathHelper.ceil(d)));
            });
         }
      }
   }
   ...
   protected void keepRunning(ServerWorld serverWorld, E mobEntity, long l) {
      if (this.lastTarget.isPresent()) {
         if (l - this.targetTime >= 40L) {
            ...
            serverWorld.playSoundFromEntity((PlayerEntity)null, mobEntity, (SoundEvent)this.field_33460.apply(mobEntity), SoundCategory.NEUTRAL, 1.0F, 1.0F);
         }
      } else {
         --this.cooldown;
         ...
         if (optional.isPresent()) {
            ...
            mobEntity.getBrain().remember(MemoryModuleType.LOOK_TARGET, (Object)(new BlockPosLookTarget(((LongJumpTask.Target)optional.get()).getPos())));
            EntityNavigation entityNavigation = mobEntity.getNavigation();
   ...
   private Optional<Vec3d> getRammingVelocity(MobEntity entity, Vec3d pos, int range) {
      ...
      if (p < 0.0D) {
         ...
         if (q > (double)this.maxRange) {
            ...
            return Optional.of((new Vec3d(r * o, s, r * n)).multiply(0.949999988079071D));
         }
      }
   }

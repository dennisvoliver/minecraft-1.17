...
public class LookTargetUtil {
   ...
   public static Optional<LivingEntity> getEntity(LivingEntity entity, MemoryModuleType<UUID> uuidMemoryModule) {
      ...
      return optional.map((uUID) -> {
         ...
         return entityx instanceof LivingEntity ? (LivingEntity)entityx : null;
      });
   }
   ...
   public static Vec3d find(PathAwareEntity entity, int horizontalRange, int verticalRange) {
      ...
      for(int var4 = 0; vec3d != null && !entity.world.getBlockState(new BlockPos(vec3d)).canPathfindThrough(entity.world, new BlockPos(vec3d), NavigationType.WATER) && var4++ < 10; vec3d = NoPenaltyTargeting.find(entity, horizontalRange, verticalRange)) {
      }

...
public class SleepTask extends Task<LivingEntity> {
   ...
   protected boolean shouldRun(ServerWorld world, LivingEntity entity) {
      if (entity.hasVehicle()) {
         ...
         if (world.getRegistryKey() != globalPos.getDimension()) {
            ...
            return globalPos.getPos().isWithinDistance(entity.getPos(), 2.0D) && blockState.isIn(BlockTags.BEDS) && !(Boolean)blockState.get(BedBlock.OCCUPIED);
         }
      }
   }
   ...
   protected void run(ServerWorld world, LivingEntity entity, long time) {
      if (time > this.startTime) {
         OpenDoorsTask.pathToDoor(world, entity, (PathNode)null, (PathNode)null);

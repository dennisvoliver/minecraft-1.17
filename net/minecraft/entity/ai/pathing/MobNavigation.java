...
public class MobNavigation extends EntityNavigation {
   ...
   public Path findPathTo(BlockPos target, int distance) {
      ...
      if (this.world.getBlockState(target).isAir()) {
         for(blockPos2 = target.down(); blockPos2.getY() > this.world.getBottomY() && this.world.getBlockState(blockPos2).isAir(); blockPos2 = blockPos2.down()) {
         }
         ...
         while(blockPos2.getY() < this.world.getTopY() && this.world.getBlockState(blockPos2).isAir()) {
      ...
      if (!this.world.getBlockState(target).getMaterial().isSolid()) {
         ...
         for(blockPos2 = target.up(); blockPos2.getY() < this.world.getTopY() && this.world.getBlockState(blockPos2).getMaterial().isSolid(); blockPos2 = blockPos2.up()) {
         }
   ...
   private boolean allVisibleArePassable(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3d entityPos, double lookVecX, double lookVecZ) {
      Iterator var12 = BlockPos.iterate(new BlockPos(x, y, z), new BlockPos(x + sizeX - 1, y + sizeY - 1, z + sizeZ - 1)).iterator();

...
public class BirdPathNodeMaker extends LandPathNodeMaker {
   ...
   public PathNode getStart() {
      ...
      PathNodeType pathNodeType = this.getNodeType(blockPos.getX(), j, blockPos.getZ());
      if (this.entity.getPathfindingPenalty(pathNodeType) < 0.0F) {
         Set<BlockPos> set = ImmutableSet.of(new BlockPos(this.entity.getBoundingBox().minX, (double)j, this.entity.getBoundingBox().minZ), new BlockPos(this.entity.getBoundingBox().minX, (double)j, this.entity.getBoundingBox().maxZ), new BlockPos(this.entity.getBoundingBox().maxX, (double)j, this.entity.getBoundingBox().minZ), new BlockPos(this.entity.getBoundingBox().maxX, (double)j, this.entity.getBoundingBox().maxZ));
         Iterator var5 = set.iterator();

         while(var5.hasNext()) {
            BlockPos blockPos2 = (BlockPos)var5.next();
            PathNodeType pathNodeType2 = this.getNodeType(blockPos.getX(), j, blockPos.getZ());
   ...
   private PathNodeType getNodeType(int x, int y, int z) {
      return (PathNodeType)this.pathNodes.computeIfAbsent(BlockPos.asLong(x, y, z), (l) -> {
         return this.getNodeType(this.cachedWorld, x, y, z, this.entity, this.entityBlockXSize, this.entityBlockYSize, this.entityBlockZSize, this.canOpenDoors(), this.canEnterOpenDoors());
      });
   }

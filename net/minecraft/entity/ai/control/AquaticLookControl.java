   ...
   public void tick() {
      if (this.active) {
         ...
         this.getTargetYaw().ifPresent((float_) -> {
            this.entity.headYaw = this.changeAngle(this.entity.headYaw, float_ + 20.0F, this.yawSpeed);
         });
         this.getTargetPitch().ifPresent((float_) -> {
            this.entity.setPitch(this.changeAngle(this.entity.getPitch(), float_ + 10.0F, this.pitchSpeed));
         });
      } else {

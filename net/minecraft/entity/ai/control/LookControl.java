...
public class LookControl implements Control {
   ...
   public void tick() {
      ...
      if (this.active) {
         ...
         this.getTargetYaw().ifPresent((float_) -> {
            this.entity.headYaw = this.changeAngle(this.entity.headYaw, float_, this.yawSpeed);
         });
         this.getTargetPitch().ifPresent((float_) -> {
            this.entity.setPitch(this.changeAngle(this.entity.getPitch(), float_, this.pitchSpeed));
         });
      } else {
   ...
   protected Optional<Float> getTargetPitch() {
      ...
      return !(Math.abs(e) > 9.999999747378752E-6D) && !(Math.abs(g) > 9.999999747378752E-6D) ? Optional.empty() : Optional.of((float)(-(MathHelper.atan2(e, g) * 57.2957763671875D)));
   }

   protected Optional<Float> getTargetYaw() {
      double d = this.lookX - this.entity.getX();
      double e = this.lookZ - this.entity.getZ();
      return !(Math.abs(e) > 9.999999747378752E-6D) && !(Math.abs(d) > 9.999999747378752E-6D) ? Optional.empty() : Optional.of((float)(MathHelper.atan2(e, d) * 57.2957763671875D) - 90.0F);
   }

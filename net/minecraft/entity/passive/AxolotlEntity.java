   ...
   public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable NbtCompound entityNbt) {
      ...
      if (spawnReason == SpawnReason.BUCKET) {
         ...
         if (entityData instanceof AxolotlEntity.AxolotlData) {
            ...
            entityData = new AxolotlEntity.AxolotlData(new AxolotlEntity.Variant[]{AxolotlEntity.Variant.getRandomAll(this.world.random), AxolotlEntity.Variant.getRandomAll(this.world.random)});
         }
   ...
   public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
      ...
      if (axolotlEntity != null) {
         ...
         if (shouldBabyBeDifferent(this.random)) {
            variant2 = AxolotlEntity.Variant.getRandomNatural(this.random);
         } else {
   ...
   public static enum Variant {
      ...
      private static AxolotlEntity.Variant getRandom(Random random, boolean includeUnnatural) {
         AxolotlEntity.Variant[] variants = (AxolotlEntity.Variant[])Arrays.stream(VARIANTS).filter((variant) -> {
            return variant.natural == includeUnnatural;
         }).toArray((i) -> {
            return new AxolotlEntity.Variant[i];
         });

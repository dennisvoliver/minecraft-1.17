...
public abstract class HorseBaseEntity extends AnimalEntity implements InventoryChangedListener, JumpingMount, Saddleable {
   ...
   private static final Predicate<LivingEntity> IS_BRED_HORSE = (entity) -> {
   ...
   public int addTemper(int difference) {
      int i = MathHelper.clamp((int)(this.getTemper() + difference), (int)0, (int)this.getMaxTemper());
      this.setTemper(i);
      return i;
   }
   ...
   public StackReference getStackReference(int mappedIndex) {
      ...
      if (i >= 0 && i < 2 && i < this.items.size()) {
         if (i == 0) {
            return this.createInventoryStackReference(i, (stack) -> {
   ...
   static {
      PARENT_HORSE_PREDICATE = TargetPredicate.createNonAttackable().setBaseMaxDistance(16.0D).ignoreVisibility().setPredicate(IS_BRED_HORSE);

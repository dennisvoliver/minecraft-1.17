...
public class GoatBrain {
   ...
   private static final TargetPredicate RAM_TARGET_PREDICATE = TargetPredicate.createAttackable().setPredicate((livingEntity) -> {
      return !livingEntity.getType().equals(EntityType.GOAT) && livingEntity.world.getWorldBorder().contains(livingEntity.getBoundingBox());
   });
   ...
   protected static void resetLongJumpCooldown(GoatEntity goat) {
      goat.getBrain().remember(MemoryModuleType.LONG_JUMP_COOLING_DOWN, (Object)LONG_JUMP_COOLDOWN_RANGE.get(goat.world.random));
      goat.getBrain().remember(MemoryModuleType.RAM_COOLDOWN_TICKS, (Object)RAM_COOLDOWN_RANGE.get(goat.world.random));
   }
   ...
   private static void addIdleActivities(Brain<GoatEntity> brain) {
      brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, new TimeLimitedTask(new FollowMobTask(EntityType.PLAYER, 6.0F), UniformIntProvider.create(30, 60))), Pair.of(0, new BreedTask(EntityType.GOAT, 1.0F)), Pair.of(1, new TemptTask((livingEntity) -> {
         return 1.25F;
      })), Pair.of(2, new WalkTowardClosestAdultTask(WALKING_SPEED, 1.25F)), Pair.of(3, new RandomTask(ImmutableList.of(Pair.of(new StrollTask(1.0F), 2), Pair.of(new GoTowardsLookTarget(1.0F, 3), 2), Pair.of(new WaitTask(30, 60), 1))))), ImmutableSet.of(Pair.of(MemoryModuleType.RAM_TARGET, MemoryModuleState.VALUE_ABSENT), Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryModuleState.VALUE_ABSENT)));
   }

   private static void addLongJumpActivities(Brain<GoatEntity> brain) {
      brain.setTaskList(Activity.LONG_JUMP, ImmutableList.of(Pair.of(0, new LeapingChargeTask(LONG_JUMP_COOLDOWN_RANGE, SoundEvents.ENTITY_GOAT_STEP)), Pair.of(1, new LongJumpTask(LONG_JUMP_COOLDOWN_RANGE, 5, 5, 1.5F, (goatEntity) -> {
         return goatEntity.isScreaming() ? SoundEvents.ENTITY_GOAT_SCREAMING_LONG_JUMP : SoundEvents.ENTITY_GOAT_LONG_JUMP;
   ...
   private static void addRamActivities(Brain<GoatEntity> brain) {
      ...
      }, RAM_TARGET_PREDICATE, (goat) -> {
         return goat.isBaby() ? 1 : 2;
      }, 3.0F, (goatEntity) -> {
         return goatEntity.isBaby() ? 1.0D : 2.5D;
      }, (goatEntity) -> {

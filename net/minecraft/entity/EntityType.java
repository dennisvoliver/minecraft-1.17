...
public class EntityType<T extends Entity> implements TypeFilter<Entity, T> {
   ...
   public T create(ServerWorld world, @Nullable NbtCompound itemNbt, @Nullable Text name, @Nullable PlayerEntity player, BlockPos pos, SpawnReason spawnReason, boolean alignPosition, boolean invertY) {
      ...
      if (entity == null) {
         ...
         if (entity instanceof MobEntity) {
            ...
            mobEntity.initialize(world, world.getLocalDifficulty(mobEntity.getBlockPos()), spawnReason, (EntityData)null, itemNbt);
   ...
   public static Optional<Entity> getEntityFromNbt(NbtCompound nbt, World world) {
      return Util.ifPresentOrElse(fromNbt(nbt).map((entityType) -> {
         ...
         LOGGER.warn((String)"Skipping Entity with id {}", (Object)nbt.getString("id"));
      });
   }
   ...
   public static Stream<Entity> streamFromNbt(final List<? extends NbtElement> entityNbtList, final World world) {
      ...
      return StreamSupport.stream(new Spliterator<Entity>() {
         public boolean tryAdvance(Consumer<? super Entity> consumer) {
            return spliterator.tryAdvance((nbtElement) -> {
               EntityType.loadEntityWithPassengers((NbtCompound)nbtElement, world, (entity) -> {
   ...
   static {
      ...
      AXOLOTL = register("axolotl", EntityType.Builder.create(AxolotlEntity::new, SpawnGroup.UNDERGROUND_WATER_CREATURE).setDimensions(0.75F, 0.42F).maxTrackingRange(10));

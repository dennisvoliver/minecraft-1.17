...
public class LocationPredicate {
   ...
   public static LocationPredicate biome(RegistryKey<Biome> biome) {
      return new LocationPredicate(NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, biome, (StructureFeature)null, (RegistryKey)null, (Boolean)null, LightPredicate.ANY, BlockPredicate.ANY, FluidPredicate.ANY);
   }
   ...
   public boolean test(ServerWorld serverWorld, double d, double e, double f) {
      if (!this.x.test(d)) {
         ...
         if (!optional.isPresent()) {
            ...
            return false;
         } else if (this.feature != null && (!bl || !serverWorld.getStructureAccessor().getStructureAt(blockPos, true, this.feature).hasChildren())) {
            return false;
         } else if (this.smokey != null && (!bl || this.smokey != CampfireBlock.isLitCampfireInRange(serverWorld, blockPos))) {
            return false;
         } else if (!this.light.test(serverWorld, blockPos)) {
            return false;
         } else if (!this.block.test(serverWorld, blockPos)) {
            return false;
         } else {
      ...
      if (json != null && !json.isJsonNull()) {
         ...
         if (jsonObject.has("dimension")) {
            ...
            var13 = (RegistryKey)var10000.resultOrPartial(var10001::error).map((identifierx) -> {
               return RegistryKey.of(Registry.WORLD_KEY, identifierx);
            }).orElse((Object)null);
         } else {
            var13 = null;
         }

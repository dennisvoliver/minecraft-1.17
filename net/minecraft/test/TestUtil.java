...
public class TestUtil {
   ...
   public static Collection<GameTestBatch> createBatches(Collection<TestFunction> testFunctions) {
      ...
      return (Collection)map.entrySet().stream().flatMap((entry) -> {
         ...
         return Streams.stream(Iterables.partition(collection, 100)).map((testFunctions) -> {
            return new GameTestBatch(string + ":" + mutableInt.incrementAndGet(), ImmutableList.copyOf((Collection)testFunctions), consumer, consumer2);
         });
   ...
   public static void clearTests(ServerWorld world, BlockPos pos, TestManager testManager, int radius) {
      ...
      BlockPos.stream(blockPos, blockPos2).filter((posx) -> {

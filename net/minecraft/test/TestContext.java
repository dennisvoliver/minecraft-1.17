...
public class TestContext {
   ...
   public void killAllEntities() {
      ...
      List<Entity> list = this.getWorld().getEntitiesByClass(Entity.class, box.expand(1.0D), (entity) -> {
   ...
   public ItemEntity spawnItem(Item item, float x, float y, float z) {
      ...
      itemEntity.setVelocity(0.0D, 0.0D, 0.0D);
   ...
   public <E extends MobEntity> E spawnMob(EntityType<E> type, BlockPos pos) {
      E mobEntity = (MobEntity)this.spawnEntity(type, pos);
   ...
   public void pushButton(BlockPos pos) {
      this.checkBlockState(pos, (blockStatex) -> {
         return blockStatex.isIn(BlockTags.BUTTONS);
      }, () -> {
   ...
   public void expectBlock(Block block, BlockPos pos) {
      ...
      this.checkBlock(pos, var10002, "Expected " + var10003 + ", got " + blockState.getBlock().getName().getString());
   }
   ...
   public <T extends Comparable<T>> void expectBlockProperty(BlockPos pos, Property<T> property, T value) {
      this.checkBlockState(pos, (state) -> {
         return state.contains(property) && state.get(property).equals(value);
      }, () -> {
         String var10000 = property.getName();
         return "Expected property " + var10000 + " to be " + value;
      });
   }
   ...
   public <T extends Comparable<T>> void checkBlockProperty(BlockPos pos, Property<T> property, Predicate<T> predicate, String errorMessage) {
      this.checkBlockState(pos, (blockState) -> {
   ...
   public void expectEntityAt(Entity entity, BlockPos pos) {
      ...
      list.stream().filter((entity2) -> {
         ...
         return new PositionedException("Expected " + entity.getType().getUntranslatedName(), blockPos, pos, this.test.getTick());
      });
   }
   ...
   public void expectItemAt(Item item, BlockPos pos, double radius) {
      BlockPos blockPos = this.getAbsolutePos(pos);
      List<? extends Entity> list = this.getWorld().getEntitiesByType(EntityType.ITEM, (new Box(blockPos)).expand(radius), Entity::isAlive);
   ...
   public void method_36028(Runnable runnable) {
      this.test.createTimedTaskRunner().createAndAdd(runnable).method_36080(() -> {
         return new GameTestException("Fail conditions met");
      });
   }

   public void method_36035(Runnable runnable) {
      LongStream.range(this.test.getTick(), (long)this.test.getTicksLeft()).forEach((l) -> {
         GameTestState var10000 = this.test;
         Objects.requireNonNull(runnable);
         var10000.runAtTick(l, runnable::run);
      });
   }
   ...
   public void method_35998(Consumer<BlockPos> consumer) {
      ...
      BlockPos.Mutable.stream(box.offset(0.0D, 1.0D, 0.0D)).forEach(consumer);
   }

...
public class SimpleRegistry<T> extends MutableRegistry<T> {
   ...
   public static <T> MapCodec<SimpleRegistry.RegistryManagerEntry<T>> createRegistryManagerEntryCodec(RegistryKey<? extends Registry<T>> key, MapCodec<T> entryCodec) {
      return RecordCodecBuilder.mapCodec((instance) -> {
         return instance.group(Identifier.CODEC.xmap(RegistryKey.createKeyFactory(key), RegistryKey::getValue).fieldOf("name").forGetter((registryManagerEntry) -> {
   ...
   public static <T> Codec<SimpleRegistry<T>> createCodec(RegistryKey<? extends Registry<T>> key, Lifecycle lifecycle, Codec<T> entryCodec) {
      return Codec.unboundedMap(Identifier.CODEC.xmap(RegistryKey.createKeyFactory(key), RegistryKey::getValue), entryCodec).xmap((map) -> {
         SimpleRegistry<T> simpleRegistry = new SimpleRegistry(key, lifecycle);
         map.forEach((registryKey, object) -> {
            simpleRegistry.add(registryKey, object, lifecycle);
         });
         return simpleRegistry;
      }, (simpleRegistry) -> {

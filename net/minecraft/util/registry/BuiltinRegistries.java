...
public class BuiltinRegistries {
   ...
   static {
      REGISTRIES = ROOT;
      CONFIGURED_SURFACE_BUILDER = addRegistry(Registry.CONFIGURED_SURFACE_BUILDER_KEY, () -> {
         return ConfiguredSurfaceBuilders.NOPE;
      });
      CONFIGURED_CARVER = addRegistry(Registry.CONFIGURED_CARVER_KEY, () -> {
         return ConfiguredCarvers.CAVE;
      });
      CONFIGURED_FEATURE = addRegistry(Registry.CONFIGURED_FEATURE_KEY, () -> {
         return ConfiguredFeatures.OAK;
      });
      CONFIGURED_STRUCTURE_FEATURE = addRegistry(Registry.CONFIGURED_STRUCTURE_FEATURE_KEY, () -> {
         return ConfiguredStructureFeatures.MINESHAFT;
      });
      STRUCTURE_PROCESSOR_LIST = addRegistry(Registry.STRUCTURE_PROCESSOR_LIST_KEY, () -> {
         return StructureProcessorLists.ZOMBIE_PLAINS;
      });
      STRUCTURE_POOL = addRegistry(Registry.STRUCTURE_POOL_KEY, StructurePools::initDefaultPools);
      BIOME = addRegistry(Registry.BIOME_KEY, () -> {
         return BuiltinBiomes.PLAINS;
      });
      CHUNK_GENERATOR_SETTINGS = addRegistry(Registry.CHUNK_GENERATOR_SETTINGS_KEY, ChunkGeneratorSettings::getInstance);

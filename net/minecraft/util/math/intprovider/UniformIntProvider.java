...
public class UniformIntProvider extends IntProvider {
   public static final Codec<UniformIntProvider> CODEC = RecordCodecBuilder.create((instance) -> {
      return instance.group(Codec.INT.fieldOf("min_inclusive").forGetter((provider) -> {
         ...
         return provider.max;
      })).apply(instance, (BiFunction)(UniformIntProvider::new));
   }).comapFlatMap((provider) -> {
      return provider.max < provider.min ? DataResult.error("Max must be at least min, min_inclusive: " + provider.min + ", max_inclusive: " + provider.max) : DataResult.success(provider);
   }, Function.identity());

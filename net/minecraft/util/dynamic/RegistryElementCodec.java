...
public final class RegistryElementCodec<E> implements Codec<Supplier<E>> {
   ...
   public static <E> Codec<List<Supplier<E>>> method_31194(RegistryKey<? extends Registry<E>> registryRef, Codec<E> elementCodec) {
      return Codec.either(of(registryRef, elementCodec, false).listOf(), elementCodec.xmap((object) -> {
         return () -> {
            return object;
         };
   ...
   public <T> DataResult<Pair<Supplier<E>, T>> decode(DynamicOps<T> ops, T input) {
      return ops instanceof RegistryOps ? ((RegistryOps)ops).decodeOrId(input, this.registryRef, this.elementCodec, this.allowInlineDefinitions) : this.elementCodec.decode(ops, input).map((pair) -> {

 ...
 */
public class Codecs {
   public static final Codec<Integer> field_33441 = method_36241(0, Integer.MAX_VALUE, (integer) -> {
      return "Value must be non-negative: " + integer;
   });
   public static final Codec<Integer> field_33442 = method_36241(1, Integer.MAX_VALUE, (integer) -> {
   ...
   private static <N extends Number & Comparable<N>> Function<N, DataResult<N>> method_36243(N number, N number2, Function<N, String> function) {
      return (number3) -> {
         return ((Comparable)number3).compareTo(number) >= 0 && ((Comparable)number3).compareTo(number2) <= 0 ? DataResult.success(number3) : DataResult.error((String)function.apply(number3));
      };
   }
   ...
   static final class Xor<F, S> implements Codec<Either<F, S>> {
      ...
      public <T> DataResult<Pair<Either<F, S>, T>> decode(DynamicOps<T> ops, T input) {
         DataResult<Pair<Either<F, S>, T>> dataResult = this.first.decode(ops, input).map((pair) -> {
            return pair.mapFirst(Either::left);
         });
         DataResult<Pair<Either<F, S>, T>> dataResult2 = this.second.decode(ops, input).map((pair) -> {

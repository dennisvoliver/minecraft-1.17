...
public interface DispenserBehavior {
   ...
   static void registerDefaults() {
      ...
      ItemDispenserBehavior itemDispenserBehavior = new ItemDispenserBehavior() {
         public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            try {
               ...
               LOGGER.error((String)"Error while dispensing spawn egg from dispenser at {}", (Object)pointer.getBlockPos(), (Object)var6);
      ...
      DispenserBlock.registerBehavior(Items.ARMOR_STAND, new ItemDispenserBehavior() {
         public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            EntityType.loadFromEntityNbt(world, (PlayerEntity)null, armorStandEntity, stack.getTag());
      ...
      DispenserBlock.registerBehavior(Items.FIRE_CHARGE, new ItemDispenserBehavior() {
         public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            world.spawnEntity((Entity)Util.make(smallFireballEntity, (smallFireballEntityx) -> {
               smallFireballEntityx.setItem(stack);
            }));
            stack.decrement(1);
            return stack;
         }
      ...
      DispenserBlock.registerBehavior(Items.GLASS_BOTTLE.asItem(), new FallibleItemDispenserBehavior() {
         ...
         private ItemStack tryPutFilledBottle(BlockPointer pointer, ItemStack emptyBottleStack, ItemStack filledBottleStack) {
            ...
            if (emptyBottleStack.isEmpty()) {
               pointer.getWorld().emitGameEvent((Entity)null, GameEvent.FLUID_PICKUP, pointer.getBlockPos());
   ...
   static void setEntityPosition(BlockPointer pointer, Entity entity, Direction direction) {
      entity.setPosition(pointer.getX() + (double)direction.getOffsetX() * (0.5000099999997474D - (double)entity.getWidth() / 2.0D), pointer.getY() + (double)direction.getOffsetY() * (0.5000099999997474D - (double)entity.getHeight() / 2.0D) - (double)entity.getHeight() / 2.0D, pointer.getZ() + (double)direction.getOffsetZ() * (0.5000099999997474D - (double)entity.getWidth() / 2.0D));
   }
}

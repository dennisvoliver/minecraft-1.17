...
public class RespawnAnchorBlock extends Block {
   ...
   public static void charge(World world, BlockPos pos, BlockState state) {
      world.setBlockState(pos, (BlockState)state.with(CHARGES, (Integer)state.get(CHARGES) + 1), Block.NOTIFY_ALL);
   ...
   static {
      ...
      VALID_SPAWN_OFFSETS = (new Builder()).addAll((Iterable)VALID_HORIZONTAL_SPAWN_OFFSETS).addAll(VALID_HORIZONTAL_SPAWN_OFFSETS.stream().map(Vec3i::down).iterator()).addAll(VALID_HORIZONTAL_SPAWN_OFFSETS.stream().map(Vec3i::up).iterator()).add((Object)(new Vec3i(0, 1, 0))).build();
   }
}

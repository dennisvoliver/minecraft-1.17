...
public class EndGatewayBlockEntity extends EndPortalBlockEntity {
   ...
   public static void tryTeleportingEntity(World world, BlockPos pos, BlockState state, Entity entity, EndGatewayBlockEntity blockEntity) {
      if (world instanceof ServerWorld && !blockEntity.needsCooldownBeforeTeleporting()) {
         ...
         if (blockEntity.exitPortalPos == null && world.getRegistryKey() == World.END) {
            blockPos2 = setupExitPortalLocation(serverWorld, pos);
            blockPos2 = blockPos2.up(10);
            LOGGER.debug((String)"Creating portal at {}", (Object)blockPos2);
            createPortal(serverWorld, blockPos2, EndGatewayFeatureConfig.createConfig(pos, false));
            blockEntity.exitPortalPos = blockPos2;
         }

         if (blockEntity.exitPortalPos != null) {
            blockPos2 = blockEntity.exactTeleport ? blockEntity.exitPortalPos : findBestPortalExitPos(world, blockEntity.exitPortalPos);
            Entity entity3;
            ...
            entity3.teleport((double)blockPos2.getX() + 0.5D, (double)blockPos2.getY(), (double)blockPos2.getZ() + 0.5D);
         }
   ...
   private static BlockPos findBestPortalExitPos(World world, BlockPos pos) {
      ...
      LOGGER.debug((String)"Best exit position for portal at {} is {}", (Object)pos, (Object)blockPos);
      return blockPos.up();
   }
   ...
   private static BlockPos setupExitPortalLocation(ServerWorld world, BlockPos pos) {
      ...
      if (blockPos == null) {
         blockPos = new BlockPos(vec3d.x + 0.5D, 75.0D, vec3d.z + 0.5D);
         LOGGER.debug((String)"Failed to find a suitable block to teleport to, spawning an island on {}", (Object)blockPos);
         ConfiguredFeatures.END_ISLAND.generate(world, world.getChunkManager().getChunkGenerator(), new Random(blockPos.asLong()), blockPos);
      } else {
         LOGGER.debug((String)"Found suitable block to teleport to: {}", (Object)blockPos);
      }
   ...
   private static Vec3d findTeleportLocation(ServerWorld world, BlockPos pos) {
      Vec3d vec3d = (new Vec3d((double)pos.getX(), 0.0D, (double)pos.getZ())).normalize();
      int i = true;
      Vec3d vec3d2 = vec3d.multiply(1024.0D);

      int var5;
      for(var5 = 16; !isChunkEmpty(world, vec3d2) && var5-- > 0; vec3d2 = vec3d2.add(vec3d.multiply(-16.0D))) {

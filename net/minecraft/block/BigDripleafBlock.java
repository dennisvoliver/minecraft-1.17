...
public class BigDripleafBlock extends HorizontalFacingBlock implements Fertilizable, Waterloggable {
   ...
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      ...
      return (BlockState)((BlockState)this.getDefaultState().with(WATERLOGGED, fluidState.isEqualAndStill(Fluids.WATER))).with(FACING, bl ? (Direction)blockState.get(FACING) : ctx.getPlayerFacing().getOpposite());
   }
   ...
   static {
      ...
      SHAPES_FOR_TILT = ImmutableMap.of(Tilt.NONE, Block.createCuboidShape(0.0D, 11.0D, 0.0D, 16.0D, 15.0D, 16.0D), Tilt.UNSTABLE, Block.createCuboidShape(0.0D, 11.0D, 0.0D, 16.0D, 15.0D, 16.0D), Tilt.PARTIAL, Block.createCuboidShape(0.0D, 11.0D, 0.0D, 16.0D, 13.0D, 16.0D), Tilt.FULL, VoxelShapes.empty());

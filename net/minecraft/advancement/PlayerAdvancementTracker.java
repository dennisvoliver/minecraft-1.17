...
public class PlayerAdvancementTracker {
   ...
   private void load(ServerAdvancementLoader advancementLoader) {
      if (this.advancementFile.isFile()) {
         try {
            ...
            LOGGER.error((String)"Couldn't parse player advancements in {}", (Object)this.advancementFile, (Object)var11);
         } catch (IOException var12) {
            LOGGER.error((String)"Couldn't access player advancements in {}", (Object)this.advancementFile, (Object)var12);
         }
      }
   ...
   public void save() {
      ...
      try {
         FileOutputStream outputStream = new FileOutputStream(this.advancementFile);

         try {
            OutputStreamWriter writer = new OutputStreamWriter(outputStream, Charsets.UTF_8.newEncoder());
         ...
         LOGGER.error((String)"Couldn't save player advancements to {}", (Object)this.advancementFile, (Object)var11);
      }
      ...
      if (advancementProgress.obtain(criterionName)) {
         ...
         if (!bl2 && advancementProgress.isDone()) {
            ...
            if (advancement.getDisplay() != null && advancement.getDisplay().shouldAnnounceToChat() && this.owner.world.getGameRules().getBoolean(GameRules.ANNOUNCE_ADVANCEMENTS)) {
               this.playerManager.broadcastChatMessage(new TranslatableText("chat.type.advancement." + advancement.getDisplay().getFrame().getId(), new Object[]{this.owner.getDisplayName(), advancement.toHoverableText()}), MessageType.SYSTEM, Util.NIL_UUID);
            }
         }
      }
      ...
      if (this.dirty || !this.visibilityUpdates.isEmpty() || !this.progressUpdates.isEmpty()) {
         ...
         while(var5.hasNext()) {
            ...
            if (this.visibleAdvancements.contains(advancement2)) {
               map.put(advancement2.getId(), (AdvancementProgress)this.advancementToProgress.get(advancement2));
            }
         }
